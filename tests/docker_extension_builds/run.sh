...
do
  ...
  set -x
  docker pull $image
  docker run --runtime=nvidia --rm $image /bin/bash -c 'yes | pip uninstall apex; yes | pip uninstall apex; git clone https://github.com/NVIDIA/apex.git; cd apex; pip install -v --no-cache-dir --global-option="--cpp_ext" --global-option="--cuda_ext" .'
  set +x
  exit_code=$?
  if [ $exit_code != 0 ]
  then
