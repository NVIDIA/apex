...
def test_set_cuda_rng_state(tensor_model_parallel_size):
    ...
    apex.mpu.random._set_cuda_rng_state(rng_state)
    for _ in range(5):
        torch.randn(size, out=tensor)
    apex.mpu.random._set_cuda_rng_state(rng_state)
    for _ in range(5):
...
def test_cuda_rng_tracker(tensor_model_parallel_size):
    ...
    torch.cuda.manual_seed(seed_1)
    apex.mpu.get_cuda_rng_tracker().add('test', seed_2)

    torch.randn(size, out=tensor)
    result_11 = tensor.clone()

    with apex.mpu.get_cuda_rng_tracker().fork('test'):
        torch.randn(size, out=tensor)
    ...
    # Reset the tracker
    apex.mpu.get_cuda_rng_tracker().reset()
...
def test_model_parallel_cuda_manual_seed(tensor_model_parallel_size):
    ...
    apex.mpu.model_parallel_cuda_manual_seed(12345)
    assert torch.cuda.initial_seed() == 12345
    with apex.mpu.get_cuda_rng_tracker().fork():
        assert (
            torch.cuda.initial_seed() ==
            12345 + 2718 + apex.mpu.get_tensor_model_parallel_rank()
        )
