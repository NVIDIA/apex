...
class TestFusedScaleMaskSoftmax(common_utils.TestCase):
    ...
    def test_fused_scale_mask_softmax(self):
        ...
        for dtype, scale, softmax_in_fp32, shape in itertools.product(
            ...
            attention_scores_0 = (
                torch.randn(shape).to(device="cuda", dtype=dtype).requires_grad_(True)
            )
            with torch.no_grad():
...
class TestGenericFusedSoftmaxKernel(common_utils.TestCase):
    ...
    def test_forward(self, allmasked: bool = False):
        ...
        for qlen, klen in self.q_k_lens:
            inputs = torch.normal(
                0,
                2,
                (self.batch, self.attn, qlen, klen),
                dtype=self.dtype,
                device=self.device,
            )
            masks = (
            ...
            self.assertEqual(
                softmax_results_torch.to(self.dtype),
                softmax_results,
                **self.thresh,
                msg=f"(q, k) = ({qlen, klen})",
            )
    ...
    def test_backward(self, allmasked: bool = False):
        ...
        for qlen, klen in self.q_k_lens:
            ...
            back_grad = generic_scaled_masked_softmax_cuda.backward(
                backward, softmax_results, self.scale_t
            )
