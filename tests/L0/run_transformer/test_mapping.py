    ...
    def test_reduce(self):
        for tensor_model_paralell_world_size in range(1, self.world_size + 1):
            ...
            with self.subTest(tensor_model_paralell_world_size=tensor_model_paralell_world_size):
                ...
                expected = torch.full((10, 10, 10, 10), 50 * tensor_model_paralell_world_size, device=f"cuda:{self.rank}")
    ...
    def test_split(self):
        for tensor_model_paralell_world_size in range(1, self.world_size + 1):
            ...
            with self.subTest(tensor_model_paralell_world_size=tensor_model_paralell_world_size):
                ...
                tensors = [
                    torch.randn(10, 1) for rank in range(tensor_model_paralell_world_size)
                ]
                x = torch.cat(tensors, 1)
                out = mappings._split(x)
                self.assertTrue(torch.equal(out, tensors[parallel_state.get_tensor_model_parallel_rank()]))
                parallel_state.destroy_model_parallel()
