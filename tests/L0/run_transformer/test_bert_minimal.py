...
class BertTestBase:
    ...
    def _generate_fancy_data_labels(self, sequence_len, batch_size):
        ...
        for i in range(batch_size):
            if self.inds is None or self.data_idx >= len(self.inds):
                ...
                self.inds = torch.randperm(self.effective_length, device="cuda")
                self.masks = (
                    torch.rand(
                        len(self.inds) // batch_size + 1,
                        batch_size,
                        sequence_len,
                        device="cuda",
                    )
            ...
            curr = self.fancy_data[offset : offset + sequence_len].clone().detach()
            temps.append(curr)
    ...
    def _train(
        ...
        for _ in range(16):
            batch = self._generate_fancy_data_labels(sequence_len, global_batch_size)
            optim.zero_grad()
