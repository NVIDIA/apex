...
class NcclDistributedFusedLAMB(NcclDistributedTestBase):
    ...
    def test_distributed_fused_lamb(self, no_copy):
        if no_copy == True and not 'no_copy' in inspect.getfullargspec(torch.distributed.reduce_scatter).args:
            self.skipTest("does not support no_copy")
        if no_copy == True and not 'no_copy' in inspect.getfullargspec(torch.distributed.all_gather).args:
            self.skipTest("does not support no_copy")
