...
void apex_multi_tensor_sgd(int64_t chunk_size, at::Tensor noop_flag, std::vector<std::vector<at::Tensor>> tensor_lists,
                           double wd, double momentum, double dampening, double lr, bool nesterov, bool first_run,
...
std::tuple<at::Tensor, at::Tensor> apex_multi_tensor_l2norm(int64_t chunk_size, at::Tensor noop_flag,
                                                            std::vector<std::vector<at::Tensor>> tensor_lists,
                                                            at::optional<bool> per_tensor_python) {
  return multi_tensor_l2norm_cuda(static_cast<int>(chunk_size), noop_flag, std::move(tensor_lists), per_tensor_python);
}

std::tuple<at::Tensor, at::Tensor> apex_multi_tensor_l2norm_mp(int64_t chunk_size, at::Tensor noop_flag,
                                                               std::vector<std::vector<at::Tensor>> tensor_lists,
                                                               at::optional<bool> per_tensor_python) {
...
std::tuple<at::Tensor, at::Tensor> apex_multi_tensor_l2norm_scale(int64_t chunk_size, at::Tensor noop_flag,
                                                                  std::vector<std::vector<at::Tensor>> tensor_lists,
                                                                  double scale, at::optional<bool> per_tensor_python) {
...
std::tuple<at::Tensor, at::Tensor> apex_multi_tensor_unscale_l2norm(int64_t chunk_size, at::Tensor noop_flag,
                                                                    std::vector<std::vector<at::Tensor>> tensor_lists,
                                                                    at::Tensor inv_scale,
                                                                    at::optional<bool> per_tensor_python) {
...
void apex_multi_tensor_lamb_stage1_cuda(int64_t chunk_size, at::Tensor noop_flag,
                                        std::vector<std::vector<at::Tensor>> tensor_lists, at::Tensor per_tensor_decay,
                                        int64_t step, double beta1, double beta2, double epsilon,
                                        at::Tensor global_grad_norm, double max_global_grad_norm) {
  multi_tensor_lamb_stage1_cuda(static_cast<int>(chunk_size), noop_flag, std::move(tensor_lists), per_tensor_decay,
                                ...
                                static_cast<float>(epsilon), global_grad_norm,
                                static_cast<float>(max_global_grad_norm));
}
...
void apex_multi_tensor_lamb_stage2_cuda(int64_t chunk_size, at::Tensor noop_flag,
  ...
  multi_tensor_lamb_stage2_cuda(static_cast<int>(chunk_size), noop_flag, std::move(tensor_lists), per_tensor_param_norm,
                                per_tensor_update_norm, static_cast<float>(lr), static_cast<float>(weight_decay),
                                use_nvlamb_python);
}

void apex_multi_tensor_adam(int64_t chunk_size, at::Tensor noop_flag, std::vector<std::vector<at::Tensor>> tensor_lists,
                            double lr, double beta1, double beta2, double epsilon, int64_t step, int64_t mode,
...
void apex_multi_tensor_adam_capturable_master(int64_t chunk_size, at::Tensor noop_flag,
                                              std::vector<std::vector<at::Tensor>> tensor_lists, at::Tensor lr,
                                              double beta1, double beta2, double epsilon, at::Tensor step, int64_t mode,
                                              int64_t bias_correction, double weight_decay, at::Tensor inv_scale) {
...
void apex_multi_tensor_novograd(int64_t chunk_size, at::Tensor noop_flag,
  ...
  multi_tensor_novograd_cuda(static_cast<int>(chunk_size), noop_flag, std::move(tensor_lists), grad_norms,
                             ...
                             static_cast<float>(weight_decay), static_cast<int>(grad_averaging), static_cast<int>(mode),
                             static_cast<int>(norm_type));
}

void apex_multi_tensor_lamb(int64_t chunk_size, at::Tensor noop_flag, std::vector<std::vector<at::Tensor>> tensor_lists,
                            double lr, double beta1, double beta2, double epsilon, int64_t step,
                            int64_t bias_correction, double weight_decay, int64_t grad_averaging, int64_t mode,
                            at::Tensor global_grad_norm, double max_grad_norm, at::optional<bool> use_nvlamb_python) {
...
void apex_multi_tensor_lamb_mp(int64_t chunk_size, at::Tensor noop_flag,
                               ...
                               double weight_decay, int64_t grad_averaging, int64_t mode, at::Tensor global_grad_norm,
                               at::Tensor max_grad_norm, at::optional<bool> use_nvlamb_python, at::Tensor found_inf,
                               at::Tensor inv_scale) {
...
TORCH_LIBRARY_FRAGMENT(apex, m) {
  ...
  m.def(
      "amp_multi_tensor_sgd(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, float wd, float momentum, "
      "float dampening, float lr, bool nesterov, bool first_run, bool wd_after_momentum, float scale) -> ()");
  m.def(
      "amp_multi_tensor_axpby(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, float a, float b, "
      "int arg_to_check) -> ()");
  m.def(
      "amp_multi_tensor_l2norm(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, bool? per_tensor_python) "
      "-> (Tensor, Tensor)");
  m.def(
      "amp_multi_tensor_l2norm_mp(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, "
      "bool? per_tensor_python) -> (Tensor, Tensor)");
  m.def(
      "amp_multi_tensor_l2norm_scale(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, float scale, "
      "bool? per_tensor_python) -> (Tensor, Tensor)");
  m.def(
      "amp_multi_tensor_unscale_l2norm(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, Tensor inv_scale, "
      "bool? per_tensor_python) -> (Tensor, Tensor)");
  m.def(
      "amp_multi_tensor_lamb_stage1_cuda(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, "
      "Tensor per_tensor_decay, int step, float beta1, float beta2, float epsilon, Tensor global_grad_norm, "
      "float max_global_grad_norm) -> ()");
  m.def(
      "amp_multi_tensor_lamb_stage2_cuda(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, "
      "Tensor per_tensor_param_norm, Tensor per_tensor_update_norm, float lr, float weight_decay, "
      "bool? use_nvlamb_python) -> ()");
  m.def(
      "amp_multi_tensor_adam(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, float lr, float beta1, "
      "float beta2, float epsilon, int step, int mode, int bias_correction, float weight_decay) -> ()");
  m.def(
      "amp_multi_tensor_adam_capturable(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, Tensor lr, "
      "float beta1, float beta2, float epsilon, Tensor step, int mode, int bias_correction, float weight_decay, "
      "Tensor inv_scale) -> ()");
  m.def(
      "amp_multi_tensor_adam_capturable_master(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, Tensor lr, "
      "float beta1, float beta2, float epsilon, Tensor step, int mode, int bias_correction, float weight_decay, "
      "Tensor inv_scale) -> ()");
  m.def(
      "amp_multi_tensor_adagrad(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, float lr, float epsilon, "
      "int mode, float weight_decay) -> ()");
  m.def(
      "amp_multi_tensor_novograd(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, Tensor grad_norms, "
  ...
  m.def(
      "amp_multi_tensor_lamb(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, float lr, float beta1, "
  ...
  m.def(
      "amp_multi_tensor_lamb_mp(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, Tensor lr, float beta1, "
      ...
      "Tensor inv_scale) -> ()");
  m.def(
      "amp_update_scale_hysteresis(Tensor current_scale, Tensor growth_tracker, Tensor hysteresis_tracker, "
