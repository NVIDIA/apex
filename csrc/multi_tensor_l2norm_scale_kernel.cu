...
template <typename in_t, typename out_t> struct L2NormScaleFunctor {
  __device__ __forceinline__ void
  operator()(int chunk_size, volatile int *noop_gmem, TensorListMetadata<2> &tl,
             float *output, float *output_per_tensor, float scale,
...
__global__ void cleanup_v3(float *output, float *output_per_tensor, float *ret,
  ...
  DISPATCH_FLOAT_AND_HALF(
      ...
      DISPATCH_FLOAT_AND_HALF(
          ...
          multi_tensor_apply<2>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                                ...
                                per_tensor ? output_per_tensor.data_ptr<float>()
                                           : nullptr,
                                scale, per_tensor, max_chunks_per_tensor);))
