...
std::vector<at::Tensor> mlp_forward(int use_bias, int activation, std::vector<at::Tensor> inputs) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(inputs[0].scalar_type(), "mlp_forward", [&] {
    ...
    auto result = mlp_fp<scalar_t>(inputs[0].data_ptr<scalar_t>(), input_features, batch_size, w_ptr.data(), num_layers,
                                   output_features.data(), b_ptr.data(), out.data_ptr<scalar_t>(),
                                   reserved_space.data_ptr<scalar_t>(), use_bias, activation,
...
std::vector<at::Tensor> mlp_backward(int use_bias, int activation, at::Tensor grad_o,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(inputs[0].scalar_type(), "mlp_backward", [&] {
    ...
    auto result = mlp_bp<scalar_t>(inputs[0].data_ptr<scalar_t>(), fprop_outputs[0].data_ptr<scalar_t>(),
                                   input_features, batch_size, w_ptr.data(), num_layers, output_features.data(),
                                   grad_o.contiguous().data_ptr<scalar_t>(), fprop_outputs[1].data_ptr<scalar_t>(),
                                   work_space.data_ptr<scalar_t>(), outputs_ptr[0], outputs_ptr.data() + 1,
