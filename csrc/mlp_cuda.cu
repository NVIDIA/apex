...
#define BIAS_RELU_RED_PER_THREAD                                               \
  16 // backward minimal reduction length per thread
...
template <typename T>
__device__ __forceinline__ void load_store(T *dst, volatile T *src,
...
template <typename T>
__device__ __forceinline__ void load_store(volatile T *dst, T *src,
                                           int dst_offset, int src_offset) {
  typedef
      typename std::aligned_storage<ILP * sizeof(T), ILP * alignof(T)>::type LT;
  ((LT *)dst)[dst_offset] = ((LT *)src)[src_offset];
...
cublasStatus_t mlp_gemm(cublasHandle_t handle, cublasOperation_t transa,
                        cublasOperation_t transb, int m, int n, int k,
                        float *alpha, const double *A, int lda, const double *B,
                        int ldb, const float *beta, double *C, int ldc) {
...
cublasStatus_t mlp_gemm(cublasHandle_t handle, cublasOperation_t transa,
                        cublasOperation_t transb, int m, int n, int k,
                        float *alpha, const at::Half *A, int lda,
                        const at::Half *B, int ldb, float *beta, at::Half *C,
                        int ldc) {
...
#if defined(CUBLAS_VERSION) && CUBLAS_VERSION >= 11000
int mlp_gemm_lt(cublasLtHandle_t ltHandle, cublasOperation_t transa,
                cublasOperation_t transb, int m, int n, int k,
                float *alpha, /* host pointer */
                const at::Half *A, int lda, const at::Half *B, int ldb,
                float *beta, /* host pointer */
                at::Half *C, int ldc, void *workspace, size_t workspaceSize,
                cudaStream_t stream, bool use_bias, bool use_relu,
                const void *bias) {
  ...
  status = cublasLtMatrixLayoutInit(&Adesc, CUDA_R_16F,
                                    transa == CUBLAS_OP_N ? m : k,
                                    transa == CUBLAS_OP_N ? k : m, lda);
  if (status != CUBLAS_STATUS_SUCCESS)
    goto CLEANUP;
  status = cublasLtMatrixLayoutInit(&Bdesc, CUDA_R_16F,
  ...
  status = cublasLtMatmul(ltHandle, &operationDesc, alpha, A, &Adesc, B, &Bdesc,
                          beta, C, &Cdesc, C, &Cdesc, &heuristicResult.algo,
...
int mlp_gemm_lt(cublasLtHandle_t ltHandle, cublasOperation_t transa,
                ...
                float *beta, /* host pointer */
                double *C, int ldc, void *workspace, size_t workspaceSize,
                cudaStream_t stream, bool use_bias, bool use_relu,
                const void *bias) {
  return 1;
}
...
int mlp_gemm_lt(cublasLtHandle_t ltHandle, cublasOperation_t transa,
  ...
  // Create matrix descriptors. Not setting any extra attributes.
  status = cublasLtMatrixLayoutInit(&Adesc, CUDA_R_32F,
                                    transa == CUBLAS_OP_N ? m : k,
                                    transa == CUBLAS_OP_N ? k : m, lda);
  if (status != CUBLAS_STATUS_SUCCESS)
    goto CLEANUP;
  status = cublasLtMatrixLayoutInit(&Bdesc, CUDA_R_32F,
                                    transb == CUBLAS_OP_N ? k : n,
                                    transb == CUBLAS_OP_N ? n : k, ldb);
  if (status != CUBLAS_STATUS_SUCCESS)
    goto CLEANUP;
...
#pragma unroll
      for (int ii = 0; ii < ILP; ii++) {
        float bias_sum =
            static_cast<float>(r_x[ii]) + static_cast<float>(r_b[ii]);
        r_x[ii] = bias_sum;
      }
...
__global__ void Relu_bprop(T *dY, T *Y, uint batch_size, uint features, T *dX) {
  T r_dy[ILP];
  T r_y[ILP];
  if (is_aligned(dY) && is_aligned(Y) && is_aligned(dX) &&
      features % ILP == 0) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    for (; tid * ILP < features * batch_size; tid += blockDim.x * gridDim.x) {
...
// Safe to call in-place.
template <typename T>
__global__ void Sigmoid_bprop(T *dY, T *Y, uint batch_size, uint features,
                              T *dX) {
  T r_dy[ILP];
  T r_y[ILP];
  if (is_aligned(dY) && is_aligned(Y) && is_aligned(dX) &&
...
void get_biasAddRelu_bprop_grid_size(int yfeat, int batch_size, int block_x,
                                     int block_y, int *grid_x, int *grid_y) {
...
// Addition done deterministically via a 2-pass approach. Each CTA writes out
// partial sum, and the last CTA in grid Y dimension accumulates partials
// serially and writes to result.
template <typename T, int UNROLL_FACTOR>
__global__ void biasAdd_bprop(T *dY, int features, int batch_size,
                              volatile float *intermediate, int *semaphores,
                              T *db) {
...
#pragma unroll 4
  ...
  // Increment semaphore and check if this is the last CTA in the grid_y
  // dimension. Only thread (0,0) calls this
...
// serially and writes to result.
template <typename T, int UNROLL_FACTOR>
__global__ void biasAddRelu_bprop(T *Y, T *dY, int features, int batch_size,
                                  T *dX, volatile float *intermediate,
                                  int *semaphores, T *db) {
  // The feature that this thread is responsible for
  int f = blockIdx.x * blockDim.x + threadIdx.x;
...
// serially and writes to result.
template <typename T, int UNROLL_FACTOR>
__global__ void biasAddRelu_bprop_aligned(T *Y, T *dY, int features,
                                          int batch_size, T *dX,
...
#pragma unroll
  ...
  // we know block size for now
  __shared__ float
      smem[BIAS_RELU_BW_NTHREADS_X * BIAS_RELU_BW_NTHREADS_Y * ILP];
  // naive block reduction on y-dim
  int linear_idx = threadIdx.y * blockDim.x + threadIdx.x;
...
// Lists where the num_layers-1 intermediate Y buffers start in reserved space
// on fprop, starting offset 0. The last Y value is, of course, stored in the
// user provided output buffer.
void get_y_offsets(int batch_size, int num_layers, const int *output_features,
                   int *y_start_offsets) {
  y_start_offsets[0] = 0;
  for (int i = 1; i < num_layers; i++) {
    y_start_offsets[i] =
        y_start_offsets[i - 1] + batch_size * output_features[i - 1];
  }
}
...
size_t get_mlp_reserved_space(int64_t batch_size, int num_layers,
  ...
  size_t res_space = 0;
  // Need to store output of every intermediate MLP - size equal to
  // output_features[i] * batch_size for all 'i' in [0, num_layers-1)
...
size_t get_all_activations_size(int64_t batch_size, int num_layers,
                                const int *output_features) {
  size_t acts_size = 0;
  for (int l = 0; l < num_layers; l++) {
...
size_t get_reduction_scratch_space(int batch_size, int num_layers,
                                   const int *output_features) {
  ...
  for (int l = 0; l < num_layers; l++) {
    ...
    get_biasAddRelu_bprop_grid_size(output_features[l], batch_size, block_x,
                                    block_y, &tmp, &res0);
    ...
    max_scratch_space =
        std::max(max_scratch_space, (size_t)(output_features[l] * res0));
    max_scratch_space =
        std::max(max_scratch_space, (size_t)(output_features[l] * res1));
  }
...
// Returns the work space (in elements) needed for the MLP bprop.
template <typename T>
size_t get_mlp_bp_workspace_in_bytes(int batch_size, int num_layers,
                                     const int *output_features) {
  size_t work_space = 0;

  // Store each intermediate dY explicitly. Need 2 dYs per MLP layer (one for
  // o/p of biasReLU_bp and one for o/p of dgrad GEMM).
  work_space +=
      2 * get_all_activations_size(batch_size, num_layers, output_features) *
      sizeof(T);
  work_space +=
      get_reduction_scratch_space(batch_size, num_layers, output_features) *
      sizeof(float);
...
template <typename T>
void partition_mlp_bp_workspace(int batch_size, int num_layers,
                                const int *output_features, void *work_space,
                                T **dy_gemms, T **dx_gemms, float **db_scratch,
                                int **semaphores) {
  /*
     Workspace is partitioned as
  ...
  *dx_gemms = *dy_gemms +
              get_all_activations_size(batch_size, num_layers, output_features);
...
// Can handle num_layers number of layers, each with its own shape. Output of
// layer i is assumed to be input of layer i+1. output_features, WPtr and BPtr
// are arrays of length num_layers, and must be in the same order i.e. WPtr[i]
// and BPtr[i] are respectively the weight and bias of layer 'i'.
template <typename T>
int mlp_fp(T *X, int input_features, int batch_size, T **WPtr, int num_layers,
           int *output_features, T **BPtr, T *Y, T *reserved_space,
  ...
  for (int layer = 0; layer < num_layers; layer++) {
    ...
    if (activation < 1) {
      cublaslt_status = mlp_gemm_lt(
          // ltHandle,
          (cublasLtHandle_t)handle, CUBLAS_OP_T, CUBLAS_OP_N, ofeat, batch_size,
          ifeat, &one, weight, ifeat, input, ifeat, &zero, output, ofeat,
          lt_workspace, 1 << 22, stream, use_bias == 1, activation == 1, bias);
    }
#endif
    ...
    if (cublaslt_status != 0) {
      ...
      cublas_status =
          mlp_gemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, ofeat, batch_size, ifeat,
                   &one, weight, ifeat, input, ifeat, &zero, output, ofeat);
      ...
      if (use_bias == 1) {
        if (activation == 0) { // no activation
          cudaOccupancyMaxActiveBlocksPerMultiprocessor(
              &num_blocks, biasAdd_fprop<T>, BIAS_RELU_FW_NTHREADS, 0);
          biasAdd_fprop<<<num_SMs * num_blocks, BIAS_RELU_FW_NTHREADS, 0,
                          stream>>>(output, bias, batch_size, input_size);
        } else if (activation == 1) { // relu
          cudaOccupancyMaxActiveBlocksPerMultiprocessor(
              &num_blocks, biasAddRelu_fprop<T>, BIAS_RELU_FW_NTHREADS, 0);
          biasAddRelu_fprop<<<num_SMs * num_blocks, BIAS_RELU_FW_NTHREADS, 0,
                              stream>>>(output, bias, batch_size, input_size);
          ...
          biasAdd_fprop<<<num_SMs * num_blocks, BIAS_RELU_FW_NTHREADS, 0,
                          stream>>>(output, bias, batch_size, input_size);
          cudaOccupancyMaxActiveBlocksPerMultiprocessor(
              &num_blocks, Sigmoid_fprop<T>, BIAS_RELU_FW_NTHREADS, 0);
          Sigmoid_fprop<<<num_SMs * num_blocks, BIAS_RELU_FW_NTHREADS, 0,
                          stream>>>(output, batch_size, input_size);
        }
      } else {
        // don't need to do anything in case of no activation and no bias
        if (activation == 1) { // relu
          cudaOccupancyMaxActiveBlocksPerMultiprocessor(
              &num_blocks, Relu_fprop<T>, BIAS_RELU_FW_NTHREADS, 0);
          Relu_fprop<<<num_SMs * num_blocks, BIAS_RELU_FW_NTHREADS, 0,
                       stream>>>(output, batch_size, input_size);
        } else if (activation == 2) { // sigmoid
          cudaOccupancyMaxActiveBlocksPerMultiprocessor(
              &num_blocks, Sigmoid_fprop<T>, BIAS_RELU_FW_NTHREADS, 0);
...
template <typename T>
int mlp_bp(T *X, T *Y, int input_features, int batch_size, T **WPtr,
           int num_layers, int *output_features, T *dY, T *reserved_space,
           T *work_space, T *dX, T **dwPtr, T **dbPtr, bool requires_grad,
           int use_bias, int activation) {
  T *weight;
  ...
  partition_mlp_bp_workspace<T>(batch_size, num_layers, output_features,
                                work_space, &dy_gemm_base, &dx_gemm_base,
                                &db_scratch, &semaphores);

  size_t semaphore_size =
      get_semaphores_size(num_layers, output_features) * sizeof(int);
  ...
  for (int layer = num_layers - 1; layer >= 0; layer--) {
    ...
    if (use_bias == 1) {
      if (activation == 0) { // no acitvation
        ...
        int block_x = BIAS_RELU_BW_NTHREADS_X;
        int block_y = BIAS_RELU_RED_PER_THREAD * BIAS_RELU_BW_NTHREADS_Y;
        get_biasAddRelu_bprop_grid_size(yfeat, batch_size, block_x, block_y,
                                        &grid_x, &grid_y);
        ...
        if (yfeat % (ILP * BIAS_RELU_BW_NTHREADS_X) == 0 && is_aligned(y) &&
            is_aligned(dy) && is_aligned(dy_gemm) && is_aligned(dbias)) {
        ...
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &num_blocks, Sigmoid_bprop<T>, BIAS_RELU_FW_NTHREADS, 0);
        Sigmoid_bprop<<<num_SMs * num_blocks, BIAS_RELU_FW_NTHREADS, 0,
                        stream>>>(dy, y, batch_size, yfeat, dy_gemm);
      ...
      if (activation == 0) {
        ...
        int num_SMs =
            at::cuda::getCurrentDeviceProperties()->multiProcessorCount;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &num_blocks, Relu_bprop<T>, BIAS_RELU_FW_NTHREADS, 0);
        Relu_bprop<<<num_SMs * num_blocks, BIAS_RELU_FW_NTHREADS, 0, stream>>>(
            dy, y, batch_size, yfeat, dy_gemm);
      } else if (activation == 2) { // sigmoid
        int num_blocks = 0;
        int num_SMs =
            at::cuda::getCurrentDeviceProperties()->multiProcessorCount;
    ...
    if (layer > 0 || requires_grad == 1) {
      cublas_status =
          mlp_gemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, xfeat, batch_size, yfeat,
                   &one, weight, xfeat, dy_gemm, yfeat, &zero, dx, xfeat);
    ...
    // Call GEMM wgrad
    cublas_status =
        mlp_gemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, xfeat, yfeat, batch_size,
                 &one, x, xfeat, dy_gemm, yfeat, &zero, dweight, xfeat);
...
template int mlp_fp<float>(float *X, int input_features, int batch_size,
                           float **WPtr, int num_layers, int *output_features,
                           float **BPtr, float *Y, float *reserved_space,
                           int use_bias, int activation, void *lt_workspace);

template int mlp_bp<float>(float *X, float *Y, int input_features,
                           int batch_size, float **WPtr, int num_layers,
                           int *output_features, float *dY,
                           float *reserved_space, float *work_space, float *dX,
                           float **dwPtr, float **dbPtr, bool requires_grad,
                           int use_bias, int activation);

template int mlp_fp<at::Half>(at::Half *X, int input_features, int batch_size,
                              at::Half **WPtr, int num_layers,
                              int *output_features, at::Half **BPtr,
                              at::Half *Y, at::Half *reserved_space,
                              int use_bias, int activation, void *lt_workspace);

template int mlp_bp<at::Half>(at::Half *X, at::Half *Y, int input_features,
                              int batch_size, at::Half **WPtr, int num_layers,
                              int *output_features, at::Half *dY,
                              at::Half *reserved_space, at::Half *work_space,
                              at::Half *dX, at::Half **dwPtr, at::Half **dbPtr,
                              bool requires_grad, int use_bias, int activation);

template int mlp_fp<double>(double *X, int input_features, int batch_size,
                            double **WPtr, int num_layers, int *output_features,
                            double **BPtr, double *Y, double *reserved_space,
                            int use_bias, int activation, void *lt_workspace);

template int mlp_bp<double>(double *X, double *Y, int input_features,
                            int batch_size, double **WPtr, int num_layers,
                            int *output_features, double *dY,
                            double *reserved_space, double *work_space,
                            double *dX, double **dwPtr, double **dbPtr,
                            bool requires_grad, int use_bias, int activation);

template size_t
get_mlp_bp_workspace_in_bytes<float>(int batch_size, int num_layers,
                                     const int *output_features);
template size_t
get_mlp_bp_workspace_in_bytes<at::Half>(int batch_size, int num_layers,
                                        const int *output_features);
template size_t
get_mlp_bp_workspace_in_bytes<double>(int batch_size, int num_layers,
                                      const int *output_features);
