 ...
 * features 1) input scaling
 */
template <typename input_t, typename output_t, typename acc_t,
          int log2_elements>
__global__ void scaled_softmax_warp_forward(output_t *dst, const input_t *src,
                                            const acc_t scale,
...
__global__ void scaled_masked_softmax_warp_forward(
  ...
  long int thread_offset_src_dst =
      first_batch * element_count + ELEMENTS_PER_LDG_STG * local_idx;
  long int thread_offset_mask =
      pad_first_batch * element_count + ELEMENTS_PER_LDG_STG * local_idx;
