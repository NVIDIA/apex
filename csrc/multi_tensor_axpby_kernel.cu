...
template <typename x_t, typename y_t, typename out_t> struct AxpbyFunctor {
  __device__ __forceinline__ void operator()(int chunk_size,
                                             volatile int *noop_gmem,
                                             TensorListMetadata<3> &tl, float a,
                                             float b, int arg_to_check) {
...
#pragma unroll
        for (int ii = 0; ii < ILP; ii++) {
          r_out[ii] =
              a * static_cast<float>(r_x[ii]) + b * static_cast<float>(r_y[ii]);
          if (arg_to_check == -1)
...
void multi_tensor_axpby_cuda(int chunk_size, at::Tensor noop_flag,
      ...
      DISPATCH_FLOAT_AND_HALF(
          ...
          DISPATCH_FLOAT_AND_HALF(
              ...
              multi_tensor_apply<3>(
                  ...
                  AxpbyFunctor<scalar_t_0, scalar_t_1, scalar_t_2>(), a, b,
                  arg_to_check);)))
