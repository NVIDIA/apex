...
def forward(input, mask, scale_factor):
    return torch.ops.apex.generic_scaled_masked_softmax_forward(input, mask, scalar_float(scale_factor))


def backward(output_grads, softmax_results, scale_factor):
    return torch.ops.apex.generic_scaled_masked_softmax_backward(output_grads, softmax_results, scalar_float(scale_factor))
