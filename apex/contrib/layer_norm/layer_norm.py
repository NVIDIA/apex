...
class FastLayerNorm1pFN(torch.autograd.Function):
    ...
    def forward(ctx, x, gamma, beta, epsilon):
        ...
        hidden_size = gamma.numel()
        xmat = x.view((-1, hidden_size))
        ymat, mu, rsigma = fast_layer_norm.ln_fwd(xmat, gamma, beta, epsilon, 1.0)
        ctx.save_for_backward(x, gamma, mu, rsigma)
        return ymat.view(x.shape)
    ...
    def backward(ctx, dy):
        ...
        xmat = x.view((-1, hidden_size))
        dymat = dy.view(xmat.shape)
        dxmat, dgamma, dbeta, _, _ = fast_layer_norm.ln_bwd(dymat, xmat, mu, rsigma, gamma, 1.0)
        dx = dxmat.view(x.shape)
        return dx, dgamma, dbeta, None


def _fast_layer_norm_1p(x, weight, bias, epsilon):
    args = _cast_if_autocast_enabled(x, weight, bias, epsilon)
    with torch.cuda.amp.autocast(enabled=False):
...
class FastLayerNorm1p(torch.nn.Module):
    ...
    def forward(self, x):
        return _fast_layer_norm_1p(x, self.weight, self.bias, self.epsilon)