...
std::vector<at::Tensor> fwd(bool use_mask, bool use_time_mask, bool is_training, int heads,
                               at::Tensor const& inputs_q, at::Tensor const& inputs_kv,
                               at::Tensor const& input_weights_q, at::Tensor const& input_weights_kv,
                               at::Tensor const& output_weights, at::Tensor const& pad_mask, float dropout_prob) {
...
std::vector<at::Tensor> fwd(bool use_mask, bool use_time_mask, bool is_training, int heads,
                               at::Tensor const& inputs, at::Tensor const& input_weights,
                               at::Tensor const& output_weights, at::Tensor const& pad_mask, float dropout_prob) {
...
std::vector<at::Tensor> fwd(bool use_mask, bool use_time_mask, bool is_training, int heads,
                               at::Tensor const& inputs, at::Tensor const& input_weights,
                               at::Tensor const& output_weights, at::Tensor const& input_biases,
                               at::Tensor const& output_biases, at::Tensor const& pad_mask, float dropout_prob) {
  TORCH_CHECK(inputs.dim() == 3, "expected 3D tensor");
  TORCH_CHECK(input_weights.dim() == 2, "expected 2D tensor");
  TORCH_CHECK(output_weights.dim() == 2, "expected 2D tensor");
...
std::vector<at::Tensor> bwd(int heads, at::Tensor const& output_grads, at::Tensor const& matmul2_results,
                               ...
                               at::Tensor const& pad_mask, at::Tensor const& input_lin_results,
                               at::Tensor const& inputs, at::Tensor const& input_weights,
                               at::Tensor const& output_weights, at::Tensor const& dropout_mask,
                               float dropout_prob) {
  TORCH_CHECK(output_grads.dim() == 3, "expected 3D tensor");
...
std::vector<at::Tensor> fwd_cuda(bool use_time_mask, bool is_training, int heads, at::Tensor const& inputs,
                                    at::Tensor const& lyr_nrm_gamma_weights,
                                    at::Tensor const& lyr_nrm_beta_weights, at::Tensor const& input_weights,
                                    at::Tensor const& output_weights, const uint8_t* pad_mask, float dropout_prob);
...
std::vector<at::Tensor> apex_fast_multihead_attn_additive_mask_softmax_dropout_forward(
    bool use_mask, bool is_training, int64_t heads, at::Tensor const& input, at::Tensor const& pad_mask,
    double dropout_prob) {
  return multihead_attn::fused_softmax::additive_mask_softmax_dropout::fwd(
      use_mask, is_training, as_int(heads), input, pad_mask, as_float(dropout_prob));
}

at::Tensor apex_fast_multihead_attn_additive_mask_softmax_dropout_backward(
    bool use_mask, int64_t heads, at::Tensor const& output_grads, at::Tensor const& softmax_results,
...
std::vector<at::Tensor> apex_fast_multihead_attn_mask_softmax_dropout_forward(
    bool use_mask, bool is_training, int64_t heads, at::Tensor const& input, at::Tensor const& pad_mask,
    double dropout_prob) {
  return multihead_attn::fused_softmax::mask_softmax_dropout::fwd(
      use_mask, is_training, as_int(heads), input, pad_mask, as_float(dropout_prob));
}

at::Tensor apex_fast_multihead_attn_mask_softmax_dropout_backward(
    bool use_mask, int64_t heads, at::Tensor const& output_grads, at::Tensor const& softmax_results,
    at::Tensor const& dropout_mask, at::Tensor const& padding_mask, double dropout_prob) {
...
std::vector<at::Tensor> apex_fast_multihead_attn_encdec_multihead_attn_backward(
    int64_t heads, at::Tensor const& output_grads, at::Tensor const& matmul2_results,
    at::Tensor const& dropout_results, at::Tensor const& softmax_results,
    at::Tensor const& input_lin_q_results, at::Tensor const& input_lin_kv_results,
    ...
    double dropout_prob) {
  return multihead_attn::encdec::cublas_gemmex::bwd(
      as_int(heads), output_grads, matmul2_results, dropout_results, softmax_results, input_lin_q_results,
      input_lin_kv_results, inputs_q, inputs_kv, input_weights_q, input_weights_kv, output_weights, dropout_mask,
...
std::vector<at::Tensor> apex_fast_multihead_attn_encdec_multihead_attn_norm_add_forward(
    ...
    at::Tensor const& inputs_kv, at::Tensor const& lyr_nrm_gamma_weights,
    at::Tensor const& lyr_nrm_beta_weights, at::Tensor const& input_weights_q,
    at::Tensor const& input_weights_kv, at::Tensor const& output_weights, at::Tensor const& pad_mask,
    double dropout_prob) {
...
std::vector<at::Tensor> apex_fast_multihead_attn_encdec_multihead_attn_norm_add_backward(
    ...
    at::Tensor const& lyr_nrm_beta_weights, at::Tensor const& input_weights_q,
    at::Tensor const& input_weights_kv, at::Tensor const& output_weights, at::Tensor const& dropout_mask,
    at::Tensor const& dropout_add_mask, double dropout_prob) {
  return multihead_attn::encdec_norm_add::cublas_gemmex::bwd(
      as_int(heads), output_grads, matmul2_results, dropout_results, softmax_results, input_lin_q_results,
      input_lin_kv_results, lyr_nrm_results, lyr_nrm_mean, lyr_nrm_invvar, inputs_q, inputs_kv,
      lyr_nrm_gamma_weights, lyr_nrm_beta_weights, input_weights_q, input_weights_kv, output_weights, dropout_mask,
      dropout_add_mask, as_float(dropout_prob));
}

std::vector<at::Tensor> apex_fast_multihead_attn_self_attn_forward(
    bool use_mask, bool use_time_mask, bool is_training, int64_t heads, at::Tensor const& inputs,
    at::Tensor const& input_weights, at::Tensor const& output_weights, at::Tensor const& pad_mask,
    double dropout_prob) {
...
std::vector<at::Tensor> apex_fast_multihead_attn_self_attn_backward(
    int64_t heads, at::Tensor const& output_grads, at::Tensor const& matmul2_results,
    at::Tensor const& dropout_results, at::Tensor const& softmax_results,
    at::Tensor const& input_lin_results, at::Tensor const& inputs, at::Tensor const& input_weights,
    at::Tensor const& output_weights, at::Tensor const& dropout_mask, double dropout_prob) {
...
std::vector<at::Tensor> apex_fast_multihead_attn_self_attn_bias_backward(
    ...
    at::Tensor const& input_lin_results, at::Tensor const& inputs, at::Tensor const& input_weights,
    at::Tensor const& output_weights, at::Tensor const& dropout_mask, double dropout_prob) {
  return multihead_attn::self_bias::cublas_gemmex::bwd(
      as_int(heads), output_grads, matmul2_results, dropout_results, softmax_results, input_lin_results, inputs,
...
std::vector<at::Tensor> apex_fast_multihead_attn_self_attn_bias_additive_mask_backward(
    int64_t heads, at::Tensor const& output_grads, at::Tensor const& matmul2_results,
    at::Tensor const& dropout_results, at::Tensor const& bmm1_results, at::Tensor const& pad_mask,
    at::Tensor const& input_lin_results, at::Tensor const& inputs, at::Tensor const& input_weights,
    at::Tensor const& output_weights, at::Tensor const& dropout_mask, double dropout_prob) {
...
std::vector<at::Tensor> apex_fast_multihead_attn_self_attn_norm_add_forward(
    ...
    at::Tensor const& input_weights, at::Tensor const& output_weights, at::Tensor const& pad_mask,
    double dropout_prob) {
  return multihead_attn::self_norm_add::cublas_gemmex::fwd(
      use_mask, use_time_mask, is_training, as_int(heads), inputs, lyr_nrm_gamma_weights, lyr_nrm_beta_weights,
      input_weights, output_weights, pad_mask, as_float(dropout_prob));
}
...
std::vector<at::Tensor> apex_fast_multihead_attn_self_attn_norm_add_backward(
  ...
  return multihead_attn::self_norm_add::cublas_gemmex::bwd(
      ...
      lyr_nrm_results, lyr_nrm_mean, lyr_nrm_invvar, inputs, lyr_nrm_gamma_weights, lyr_nrm_beta_weights,
      input_weights, output_weights, dropout_mask, dropout_add_mask, as_float(dropout_prob));
}
}  // namespace

TORCH_LIBRARY_FRAGMENT(apex, m) {
  m.def("fast_multihead_attn_additive_mask_softmax_dropout_forward(bool use_mask, bool is_training, int heads, "
        "Tensor input, Tensor pad_mask, float dropout_prob) -> Tensor[]");
  m.def("fast_multihead_attn_additive_mask_softmax_dropout_backward(bool use_mask, int heads, Tensor output_grads, "
        "Tensor softmax_results, Tensor dropout_mask, float dropout_prob) -> Tensor");
  m.def("fast_multihead_attn_mask_softmax_dropout_forward(bool use_mask, bool is_training, int heads, Tensor input, "
        "Tensor pad_mask, float dropout_prob) -> Tensor[]");
  m.def("fast_multihead_attn_mask_softmax_dropout_backward(bool use_mask, int heads, Tensor output_grads, "
        "Tensor softmax_results, Tensor dropout_mask, Tensor padding_mask, float dropout_prob) -> Tensor");
  m.def("fast_multihead_attn_encdec_multihead_attn_forward(bool use_mask, bool use_time_mask, bool is_training, "
  ...
  m.def("fast_multihead_attn_encdec_multihead_attn_backward(int heads, Tensor output_grads, Tensor matmul2_results, "
  ...
  m.def("fast_multihead_attn_encdec_multihead_attn_norm_add_forward(bool use_mask, bool use_time_mask, "
        ...
        "Tensor pad_mask, float dropout_prob) -> Tensor[]");
  m.def("fast_multihead_attn_encdec_multihead_attn_norm_add_backward(int heads, Tensor output_grads, "
  ...
  m.def("fast_multihead_attn_self_attn_forward(bool use_mask, bool use_time_mask, bool is_training, int heads, "
        "Tensor inputs, Tensor input_weights, Tensor output_weights, Tensor pad_mask, float dropout_prob) "
        "-> Tensor[]");
  m.def("fast_multihead_attn_self_attn_backward(int heads, Tensor output_grads, Tensor matmul2_results, "
  ...
  m.def("fast_multihead_attn_self_attn_bias_forward(bool use_mask, bool use_time_mask, bool is_training, int heads, "
        "Tensor inputs, Tensor input_weights, Tensor output_weights, Tensor input_biases, Tensor output_biases, "
        "Tensor pad_mask, float dropout_prob) -> Tensor[]");
  m.def("fast_multihead_attn_self_attn_bias_backward(int heads, Tensor output_grads, Tensor matmul2_results, "
        "Tensor dropout_results, Tensor softmax_results, Tensor input_lin_results, Tensor inputs, "
        "Tensor input_weights, Tensor output_weights, Tensor dropout_mask, float dropout_prob) -> Tensor[]");
  m.def("fast_multihead_attn_self_attn_bias_additive_mask_forward(bool use_mask, bool use_time_mask, "
  ...
  m.def("fast_multihead_attn_self_attn_bias_additive_mask_backward(int heads, Tensor output_grads, "
  ...
  m.def("fast_multihead_attn_self_attn_norm_add_forward(bool use_mask, bool use_time_mask, bool is_training, "
  ...
  m.def("fast_multihead_attn_self_attn_norm_add_backward(int heads, Tensor output_grads, Tensor matmul2_results, "
...
TORCH_LIBRARY_IMPL(apex, CUDA, m) {
  ...
  m.impl("fast_multihead_attn_mask_softmax_dropout_forward",
         &apex_fast_multihead_attn_mask_softmax_dropout_forward);
  m.impl("fast_multihead_attn_mask_softmax_dropout_backward",
         &apex_fast_multihead_attn_mask_softmax_dropout_backward);
  m.impl("fast_multihead_attn_encdec_multihead_attn_forward",
         &apex_fast_multihead_attn_encdec_multihead_attn_forward);
