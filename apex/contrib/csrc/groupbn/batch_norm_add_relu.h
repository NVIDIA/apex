...
public:
  ...
  void fwd(cudaStream_t stream, void *my_data, void *pair_data,
           void *pair_data2, void *pair_data3, const int bn_group,
           const int magic, const int occupancy, const int grid_dim_x,
           const bool coop);
  void dgrad(cudaStream_t stream, void *my_data, void *pair_data,
             void *pair_data2, void *pair_data3, const int bn_group,
             const int magic, const int occupancy, const int grid_dim_x,
             const bool coop);
...
protected:
  ...
  void _fwdKernelLauncher(cudaStream_t stream, NhwcBatchNormFwdParams params,
                          dim3 grid_dim, int outer_loops, const int occupancy,
                          const bool coop) {
...
#define LAUNCH_BWD_ADD_RELU_KERNEL(OUTER_LOOPS, COMPILED_FOR_OCCUPANCY, COOP)  \
  do {                                                                         \
    ...
    using BWD_ADD_RELU_FUNC =                                                  \
        decltype(nhwc_batch_norm_bwd_add_relu<                                 \
                 StorageType, THREADS_PER_CTA, THREADS_PER_PIXEL,              \
    ...
    if (COOP) {                                                                \
      cudaLaunchCooperativeKernel<BWD_ADD_RELU_FUNC>(                          \
          bwd_add_relu_func, grid_dim, THREADS_PER_CTA, &params_ptr,           \
          SMEM_SIZE_BWD, stream);                                              \
    } else {                                                                   \
      cudaLaunchKernel<BWD_ADD_RELU_FUNC>(bwd_add_relu_func, grid_dim,         \
                                          THREADS_PER_CTA, &params_ptr,        \
                                          SMEM_SIZE_BWD, stream);              \
    }                                                                          \
...
const std::vector<size_t> NhwcBatchNormAddRelu::numWorkspaceBytes() const {
  ...
  const size_t bitmask_bytes =
      elems_per_group * group_count * sizeof(unsigned int);
  ...
  return {num_mean_bytes, num_variance_bytes,
          bitmask_bytes,  size_retired_ctas(grid_y),
...
void NhwcBatchNormAddRelu::_setFwdInferenceParams(
...
void NhwcBatchNormAddRelu::fwdInference(cudaStream_t stream) {
  ...
  nhwc_batch_norm_fwd_inference<StorageType, THREADS_PER_CTA, THREADS_PER_PIXEL,
                                ELEMENTS_PER_LDG, false, true>
...
void NhwcBatchNormAddRelu::fwd(cudaStream_t stream, void *my_data,
                               void *pair_data, void *pair_data2,
                               void *pair_data3, const int bn_group,
                               const int magic, const int occupancy,
  ...
  bool ptrs_are_set =
      ...
      minibatch_variance_ != nullptr && relu_bitmask_ != nullptr &&
  ...
  _fwdKernelLauncher(stream, params, grid_dim, params.outer_loops, occupancy,
                     coop);
}

void NhwcBatchNormAddRelu::dgrad(cudaStream_t stream, void *my_data,
                                 void *pair_data, void *pair_data2,
                                 void *pair_data3, const int bn_group,
                                 const int magic, const int occupancy,
                                 const int grid_dim_x, const bool coop) {
  bool ptrs_are_set =
      ...
      dX_ != nullptr
      //      && Y_ != nullptr
      && dY_ != nullptr && dAddend_ != nullptr && dscale_ != nullptr &&
      dbias_ != nullptr && retired_ctas_ != nullptr;
  ...
  _bwdKernelLauncher(stream, params, grid_dim, params.outer_loops, occupancy,
                     coop);
}
