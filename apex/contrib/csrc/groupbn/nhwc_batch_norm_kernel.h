 ...
 * \author Shankara Rao Thejaswi Nanditale, Dick Carter, Maxim Milakov, Evgeni
 * Krimer
 */
...
// CTA margin used by cooperative launch. Can be overridden by env var
// NHWC_BATCHNORM_LAUNCH_MARGIN.
#define NHWC_BATCHNORM_LAUNCH_MARGIN_MIN 3
#define NHWC_BATCHNORM_LAUNCH_MARGIN_DEFAULT NHWC_BATCHNORM_LAUNCH_MARGIN_MIN

////////////////////////////////////////////////////////////////////////////////////////////////////

template <typename T, int ELEMENTS_PER_LDG> struct PackedStorage {
...
DEVICE_FUNCTION void ldg_stream(int (&dst)[1], const uint16_t *gmem) {
  unsigned int tmp;
  asm volatile("ld.global.cs.nc.s32 %0, [%1];"
               : "=r"(tmp)
               : "l"((const uint *)gmem));
  dst[0] = tmp;
}
...
DEVICE_FUNCTION void stg_stream(uint16_t *gmem, int (&src)[2]) {
  asm volatile("st.global.cs.v2.s32 [%0], {%1,%2};" ::"l"((uint *)gmem),
               "r"(src[0]), "r"(src[1]));
}

////////////////////////////////////////////////////////////////////////////////////////////////////

template <int N> DEVICE_FUNCTION void stg(uint16_t *gmem, float (&src)[N]) {
  int tmp[N / 2];
  from_float(tmp, src);
  stg(gmem, tmp);
}
...
DEVICE_FUNCTION void read_from_gmem(float (&dst)[2], const float *gmem,
                                    int idx) {
...
DEVICE_FUNCTION void read_from_gmem(float (&dst)[4], const float *gmem,
                                    int idx) {
...
DEVICE_FUNCTION void write_to_gmem(float *gmem, int idx,
                                   const float (&src)[2]) {
...
DEVICE_FUNCTION void scaled_write_to_gmem(float *gmem, int idx,
                                          const float (&src)[4],
                                          const float coeff) {
  reinterpret_cast<float4 *>(&gmem[4 * idx])[0] = make_float4(
      src[0] * coeff, src[1] * coeff, src[2] * coeff, src[3] * coeff);
}
...
DEVICE_FUNCTION void write_to_smem(float *smem, int idx, const float (&x)[4]) {
  reinterpret_cast<float4 *>(&smem[4 * idx])[0] =
...
template <int N> DEVICE_FUNCTION void zero_array(int (&dst)[N]) {
#pragma unroll
  for (int i = 0; i < N; ++i) {
    dst[i] = 0;
  }
...
template <int N> DEVICE_FUNCTION void zero_array(float (&dst)[N]) {
#pragma unroll
  for (int i = 0; i < N; ++i) {
    dst[i] = 0.f;
  }
...
template <int N> DEVICE_FUNCTION void add(float (&x)[N], const float (&y)[N]) {
#pragma unroll
  for (int i = 0; i < N; ++i) {
    x[i] += y[i];
  }
...
template <int N> DEVICE_FUNCTION void scale_(float (&x)[N], float scalar) {
#pragma unroll
  for (int i = 0; i < N; ++i) {
    x[i] *= scalar;
  }
...
template <typename Storage> DEVICE_FUNCTION Storage relu(Storage in) {
...
template <int N> DEVICE_FUNCTION void relu_activation(float (&x)[N]) {
#pragma unroll
  for (int i = 0; i < N; ++i) {
    x[i] = relu(x[i]);
  }
...
DEVICE_FUNCTION void parallel_sums_16x2(float *smem, float (&x)[4], int nhw,
                                        void *params_my_data,
                                        void **params_pair_datas, int off,
                                        const int magic, const int sync_iters) {
  // The size of a warp.
  ...
  // The number of reducing ops, each uses its own space : mean, var, dscale,
  // dbias
...
#pragma unroll
  ...
  // We do the final reduction each half-warp sequentially reduces the final
  // values.
  if (warp_id == 0) {
...
#pragma unroll
    for (int offset = 1;
         offset < WARPS_PER_CTA / (THREADS_PER_WARP / THREADS_PER_PIXEL);
         ++offset) {
    ...
    if (threadIdx.x < THREADS_PER_PIXEL) {
      ...
      for (int sync_iter = 0; sync_iter < sync_iters; ++sync_iter) {
        // float* params_pair_data =
        // (reinterpret_cast<float**>(params_pair_datas))[sync_iter];
        ...
        // after sums for this GPU were computed, let CTA0 broadcast the sum to
        // over GPU
        if (blockIdx.x == 0) {
          ...
          asm volatile(
              "st.global.wt.v4.b32 [%0], {%1,%2,%3,%4};" ::"l"(write_data),
              "f"(x[0]), "r"(magic), "f"(x[2]), "r"(magic));

          asm volatile(
              "st.global.wt.v4.b32 [%0], {%1,%2,%3,%4};" ::"l"(write_data + 4),
              "f"(x[1]), "r"(magic), "f"(x[3]), "r"(magic));
        }

        // now each CTA (on each GPU) reads the data written by CTA 0 of the
        // other GPU
        ...
        do {
          asm volatile("ld.volatile.global.v4.b32 {%0, %1, %2, %3}, [%4];"
                       : "=f"(other[0]), "=r"(other_flag_a), "=f"(other[2]),
                         "=r"(other_flag_b)
                       : "l"(read_data));
        ...
        do {
          asm volatile("ld.volatile.global.v4.b32 {%0, %1, %2, %3}, [%4];"
                       : "=f"(other[1]), "=r"(other_flag_a), "=f"(other[3]),
                         "=r"(other_flag_b)
                       : "l"(read_data + 4));
        } while ((other_flag_a != magic) || (other_flag_b != magic));
...
DEVICE_FUNCTION void parallel_sums(float *smem, float (&x)[ELEMENTS_PER_LDG],
                                   int nhw) {
  // The size of a warp.
  const int THREADS_PER_WARP = 32;
  // The number of warps in a CTA.
  ...
  // The warp leaders, write to SMEM.
  const int idx =
      (threadIdx.x / THREADS_PER_WARP) * THREADS_PER_PIXEL + c_in_warp;
  if (nhw_in_warp == 0) {
  ...
  // We have the running mean and running m2. Let's build the mean/var of the
  // CTA.
...
template <int THREADS_PER_PIXEL, int ELEMENTS_PER_LDG> struct ParallelSums {
  template <int THREADS_PER_CTA>
  DEVICE_FUNCTION void dispatch(float *smem, float (&x)[ELEMENTS_PER_LDG],
                                int nhw) {
    parallel_sums<THREADS_PER_CTA, THREADS_PER_PIXEL, ELEMENTS_PER_LDG>(smem, x,
                                                                        nhw);
  }
};
...
template <> struct ParallelSums<16, 4> {
  template <int THREADS_PER_CTA>
  ...
  template <int THREADS_PER_CTA>
  DEVICE_FUNCTION void dispatchX(float *smem, float (&x)[4], int nhw,
                                 void *params_my_data, void **params_pair_datas,
                                 int off, const int magic,
                                 const unsigned int &sync_iters) {
    parallel_sums_16x2<THREADS_PER_CTA>(smem, x, nhw, params_my_data,
                                        params_pair_datas, off, magic,
                                        sync_iters);
  }
};

template <> struct ParallelSums<8, 4> {
  template <int THREADS_PER_CTA>
  DEVICE_FUNCTION void dispatch(float *smem, float (&x)[4], int nhw) {
...
DEVICE_FUNCTION void inter_block_sync(int *gmem_retired_ctas,
                                      int expected_count, bool master) {
...
// No DESIRED_OCCUPANCY launch bounds needed, as this is not launched
// cooperatively
template <typename Storage, int THREADS_PER_CTA, int THREADS_PER_PIXEL,
          int ELEMENTS_PER_LDG, bool USE_RELU, bool USE_ADD_RELU>
__global__
__launch_bounds__(THREADS_PER_CTA) void nhwc_batch_norm_fwd_inference(
...
struct NhwcBatchNormFwdParams {
  ...
  // The dimensions.
  int nhw, c;
  // factor to scale sum of squared errors to get saved variance.  Must be
  // 1/nhw.
  float svar_inv_count;
  // factor to scale sum of squared errors to get running variance. Should be
  // 1/nhw or 1/(nhw-1).
...
template <typename Storage, int THREADS_PER_CTA, int THREADS_PER_PIXEL,
          int PIXELS_PER_THREAD_IN_REGISTERS, int PIXELS_PER_THREAD_IN_SMEM,
          int ELEMENTS_PER_LDG, int USE_ONLINE_APPROACH, int OUTER_LOOPS_,
          bool USE_RELU, bool USE_ADD_RELU, int DESIRED_OCCUPANCY>
__global__ __launch_bounds__(
  ...
  const int PIXELS_PER_CTA_IN_REGISTERS =
      PIXELS_PER_THREAD_IN_REGISTERS * PIXELS_PER_LDG;
  ...
  __shared__ float
      smem[THREADS_PER_PIXEL * (THREADS_PER_CTA / 32) * ELEMENTS_PER_LDG];
  ...
  PackedStorageType x_storage[PIXELS_PER_THREAD_IN_REGISTERS]
  ...
  for (int c_blk_index = blockIdx.y; c_blk_index < params.c_blks;
       c_blk_index += gridDim.y) {
    // The position in the NHW dimension where the CTA starts.
    int cta_nhw_regs = blockIdx.x * PIXELS_PER_CTA_IN_REGISTERS;
    // The position in the NHW dimension where the CTA starts for the portion in
    // SMEM.
    ...
    // Clamp thread_c so that we load from valid locations even if we don't use
    // the value
    if (!is_valid_c)
      thread_c = params.c - 4;
...
#pragma unroll
    ...
    if (OUTER_LOOPS_ != 1) {
      // We cannot load everything to store persistently, so let's makes sure
      // registers and smem are fully utilized, offset is evenly divisible by 32
      int offset = (pixels_per_iteration * OUTER_LOOPS +
                    PIXELS_PER_CTA_IN_SMEM * gridDim.x - params.nhw) &
                   ~31;
      cta_nhw_regs -= offset;
      cta_nhw_smem -= offset;
    }
...
#pragma unroll
    ...
    int pixels_in_smem =
        min(smem_nhw + PIXELS_PER_CTA_IN_SMEM, params.nhw) - max(smem_nhw, 0);
    if (pixels_in_smem > 0) {
      ...
      for (int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i) {
        ...
        float is_pixel_valid =
            (((unsigned int)idx < (unsigned int)params.nhw) && is_valid_c)
                ? 1.f
                : 0.f;

        PackedStorageType x_storage_local[PACKED_ELEMENTS_PER_LDG];
        ldg_stream(x_storage_local,
                   &gmem_src[(is_pixel_valid ? idx : 0) * params.c]);

        // The offset to store in SMEM.
        const int offset = i * THREADS_PER_CTA * PACKED_ELEMENTS_PER_LDG;
        // Store in SMEM.
        write_to_smem(&smem_storage_packed[offset], threadIdx.x,
                      x_storage_local);
        // Update the count.
...
#pragma unroll
    ...
    ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<
        THREADS_PER_CTA>(smem, m1, thread_in_cta_nhw);
    __syncthreads();
...
#pragma unroll
    ...
    ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<
        THREADS_PER_CTA>(smem, m2, thread_in_cta_nhw);
    ...
    // A given cta uses the same counter every other time through the outer
    // loop.
    int *gmem_retired_ctas =
        &params.gmem_retired_ctas[c_blk_index % (2 * gridDim.y)];
...
#pragma unroll 1
    for (int idx = threadIdx.x; idx < THREADS_PER_PIXEL * gridDim.x;
         idx += THREADS_PER_CTA) {
    ...
    if (params.sync_iters > 0) {
      ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<
          THREADS_PER_CTA>(smem, m1, thread_in_cta_nhw, params.my_data,
                           params.pair_datas, 4 * c_blk_index + 3, params.magic,
                           params.sync_iters);
    } else {
      ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<
          THREADS_PER_CTA>(smem, m1, thread_in_cta_nhw);
...
#pragma unroll 1
    for (int idx = threadIdx.x; idx < THREADS_PER_PIXEL * gridDim.x;
         idx += THREADS_PER_CTA) {
      // Read the means computed by different CTAs (again). Reuse tmp if we have
      // 1 iteration.
      ...
      float mean_diff[ELEMENTS_PER_LDG],
          inv_cta_count = 1.f / static_cast<float>(cta_count);
#pragma unroll
      for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
...
#pragma unroll
      for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
        m2[i] += tmp_var[i] +
                 mean_diff[i] * mean_diff[i] * static_cast<float>(cta_count);
      }
    }

    if (params.sync_iters > 0) {
      ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<
          THREADS_PER_CTA>(smem, m2, thread_in_cta_nhw, params.my_data,
                           params.pair_datas, 4 * c_blk_index + 2, params.magic,
                           params.sync_iters);
    } else {
      ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<
          THREADS_PER_CTA>(smem, m2, thread_in_cta_nhw);
    ...
    // Finalize the stddev.
    // becasue saved var and running var may have different denominator, we
    // don't do it here scale_(m2, inv_count);

    // store the saved mean/var
    float svarinv[ELEMENTS_PER_LDG];
    bool is_valid_for_saving =
        is_valid_c && blockIdx.x == 0 && thread_in_cta_nhw == 0;
#pragma unroll
    for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
    ...
    if (is_valid_for_saving) {
      ...
      write_to_gmem(params.gmem_saved_var, thread_c / ELEMENTS_PER_LDG,
                    svarinv);
    }
    ...
    if (params.exp_avg_factor != 1.f && is_valid_for_saving) {
      read_from_gmem(rmean, params.gmem_running_mean,
                     thread_c / ELEMENTS_PER_LDG);
      read_from_gmem(rvar, params.gmem_running_var,
                     thread_c / ELEMENTS_PER_LDG);
    }
#pragma unroll
    for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
    ...
    if (is_valid_for_saving) {
      write_to_gmem(params.gmem_running_mean, thread_c / ELEMENTS_PER_LDG,
                    rmean);
    ...
    unsigned int *const gmem_relu_bitmask =
...
#pragma unroll
      for (int i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i) {
        ...
        const bool is_valid_nhw = static_cast<unsigned int>(idx) <
                                  static_cast<unsigned int>(params.nhw);
...
#pragma unroll
          for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
            ...
            unsigned int local_relu_mask =
                __ballot_sync(0xFFFFFFFFU, rectified);
            if (lane_id == i) {
              // Thread 0 remembers the relu_mask from the first time through
              // this loop, Thread 1 the next, Thread 2 the next, and Thread 3
              // the last.
...
#pragma unroll 2
      for (int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i) {
        ...
        const int offset = i * THREADS_PER_CTA * PACKED_ELEMENTS_PER_LDG;
        PackedStorageType x_storage_local[PACKED_ELEMENTS_PER_LDG];
        read_from_smem(x_storage_local, &smem_storage_packed[offset],
                       threadIdx.x);
        float x_math[ELEMENTS_PER_LDG];
        to_float(x_math, x_storage_local);
...
template <int N>
DEVICE_FUNCTION void relu_bwd(float (&dy)[N], const float (&y)[N],
                              bool valid_data) {
#pragma unroll
  for (int j = 0; j < N; ++j) {
...
template <int N>
DEVICE_FUNCTION void relu_bwd(float (&dy)[N], const bool (&rectified)[N],
                              bool valid_data) {
#pragma unroll
  for (int j = 0; j < N; ++j) {
...
template <int N>
DEVICE_FUNCTION void relu_bwd_for_dx(float (&dy)[N], const float (&x)[N],
                                     const float (&mean_var_scale_bias)[N],
...
DEVICE_FUNCTION void bwd_dx(float (&dx)[N], const float (&dy)[N],
                            const float (&var)[N], const float (&x)[N],
                            const float (&mean)[N], const float (&dscale)[N],
                            const float (&dbias)[N], float inv_count) {
#pragma unroll
  for (int j = 0; j < N; ++j) {
...
__global__ __launch_bounds__(
  ...
  PackedStorageType x_storage[PIXELS_PER_THREAD_IN_REGISTERS]
                             [PACKED_ELEMENTS_PER_LDG];
  PackedStorageType dy_storage[PIXELS_PER_THREAD_IN_REGISTERS]
                              [PACKED_ELEMENTS_PER_LDG];
  ...
  for (int c_blk_index = blockIdx.y; c_blk_index < params.c_blks;
    ...
    if (OUTER_LOOPS_ != 1) {
      // We cannot load everything to store persistently, so let's makes sure
      // registers and smem are fully utilized
...
#pragma unroll 1
    for (int loop_i = 0; loop_i < OUTER_LOOPS; ++loop_i) {
      ...
      // Update the number of elements loaded by this CTA. TODO: Skip if <= 0!!!
      cta_count +=
          max(0, min(PIXELS_PER_CTA_IN_REGISTERS, params.nhw - nhw_regs));
...
#pragma unroll
    ...
    if (pixels_in_smem > 0) {
      ...
      for (int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i) {
        ...
        write_to_smem(&smem_storage_packed[offset], threadIdx.x,
                      x_storage_local);
        offset += PIXELS_PER_THREAD_IN_SMEM * THREADS_PER_CTA *
                  PACKED_ELEMENTS_PER_LDG;
        write_to_smem(&smem_storage_packed[offset], threadIdx.x,
                      dy_storage_local);
        // Update the count.
...
#pragma unroll
    ...
    // dscale parallel sum
    ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<
        THREADS_PER_CTA>(smem, dscale, thread_in_cta_nhw);
    __syncthreads();
    // The values in shared memory correspond to the CTA-wide sums.
    ...
    // dbias parallel sum
    ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<
        THREADS_PER_CTA>(smem, dbias, thread_in_cta_nhw);
    __syncthreads();
    // The values in shared memory correspond to the CTA-wide sums.
...
#pragma unroll
    ...
    if (params.sync_iters > 0) {
      ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<
          THREADS_PER_CTA>(smem, dscale, thread_in_cta_nhw, params.my_data,
                           params.pair_datas, 4 * c_blk_index + 1, params.magic,
                           params.sync_iters);
    } else {
      ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<
          THREADS_PER_CTA>(smem, dscale, thread_in_cta_nhw);
    ...
    if (params.sync_iters > 0) {
      ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<
          THREADS_PER_CTA>(smem, dbias, thread_in_cta_nhw, params.my_data,
                           params.pair_datas, 4 * c_blk_index + 0, params.magic,
                           params.sync_iters);
    } else {
      ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<
          THREADS_PER_CTA>(smem, dbias, thread_in_cta_nhw);
    ...
    if (is_valid_for_saving) {
      if (params.sync_iters > 0) {
        scaled_write_to_gmem(params.gmem_dscale, thread_c / ELEMENTS_PER_LDG,
                             dscale, params.wgrad_coeff);
        scaled_write_to_gmem(params.gmem_dbias, thread_c / ELEMENTS_PER_LDG,
                             dbias, params.wgrad_coeff);
      } else {
    ...
    if (pixels_in_smem > 0) {
      for (int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i) {
        ...
        if (is_valid) {
          ...
          offset += PIXELS_PER_THREAD_IN_SMEM * THREADS_PER_CTA *
                    PACKED_ELEMENTS_PER_LDG;
          read_from_smem(dy_storage_local, &smem_storage_packed[offset],
                         threadIdx.x);
          float x_math[ELEMENTS_PER_LDG], dy_math[ELEMENTS_PER_LDG];
          to_float(x_math, x_storage_local);
          to_float(dy_math, dy_storage_local);
...
__global__ __launch_bounds__(
    THREADS_PER_CTA,
    DESIRED_OCCUPANCY) void nhwc_batch_norm_bwd_relu(NhwcBatchNormBwdParams
                                                         params) {
  // The number of pixels loaded in a single LDG.
  const int PIXELS_PER_LDG = THREADS_PER_CTA / THREADS_PER_PIXEL;
  // The number of pixels computed per CTA stored in registers.
  ...
  for (int c_blk_index = blockIdx.y; c_blk_index < params.c_blks;
    ...
    zero_array(varscale);
    if (is_valid_c) {
      read_from_gmem(varscale, params.gmem_saved_var,
                     thread_c / ELEMENTS_PER_LDG);
    }
    float tmp[ELEMENTS_PER_LDG];
    zero_array(tmp);
...
#pragma unroll
    ...
    if (pixels_in_smem > 0) {
      ...
      for (int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i) {
        ...
        relu_bwd(dy_math, x_math, mean_var_scale_bias, varscale,
                 is_pixel_valid);
        bwd_update(dscale, dbias, dy_math, x_math, mean, inv_count);
      }
    }
...
__global__ __launch_bounds__(
    THREADS_PER_CTA,
    DESIRED_OCCUPANCY) void nhwc_batch_norm_bwd_add_relu(NhwcBatchNormBwdParams
                                                             params) {
  // The number of pixels loaded in a single LDG.
  const int PIXELS_PER_LDG = THREADS_PER_CTA / THREADS_PER_PIXEL;
  // The number of pixels computed per CTA stored in registers.
...
#pragma unroll
    ...
    if (pixels_in_smem > 0) {
      ...
      for (int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i) {
        const int idx = smem_nhw + thread_in_cta_nhw + i * PIXELS_PER_LDG;
        const bool is_pixel_valid_nhw = static_cast<unsigned int>(idx) <
                                        static_cast<unsigned int>(params.nhw);
...
#pragma unroll
        for (int j = 0; j < ELEMENTS_PER_LDG; ++j) {
          rectified[j] =
              ((__shfl_sync(0xFFFFFFFFU, relu_mask, j) & (1U << lane_id)) != 0);
        }
