...
template<typename Ktraits>
__global__ __launch_bounds__(Ktraits::THREADS_PER_CTA) 
void rmsnorm_fwd_kernel(FwdParams params) {
    ...
    for( int row = r; row < params.rows; row += params.ctas_per_col * ROWS_PER_CTA ) {
        ...
        index_t idx = row * Ktraits::VEC_COLS + c;
        compute_t xf[LDGS * NUM_ELTS];
        compute_t m = Zeros<compute_t>::get();
        #pragma unroll
        for( int it = 0; it < LDGS; it++ ) {
            x[it].load_from(params.x, idx);
            #pragma unroll
        ...
        auto sum = Sum<compute_t>();
        m = reducer.allreduce(m, sum);

        compute_t rs = rsqrtf(m + params.epsilon);
        ...
        for( int it = 0; it < LDGS; it++ ) {
            #pragma unroll
            for( int jt = 0; jt < NUM_ELTS; jt++ ) {
                output_t y_ij = output_t(rs * xf[it * NUM_ELTS + jt]);
                output_t g_ij = gamma[it].data.elt[jt];
                z[it].data.elt[jt] = g_ij * y_ij;
            }
            z[it].store_to(params.z, idx);
            idx += VEC_COLS_PER_LDG;
        }
