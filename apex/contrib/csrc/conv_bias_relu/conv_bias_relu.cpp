...
void run_conv_bias(int64_t *x_dim, int64_t *w_dim, int64_t *y_dim,
                   int64_t *conv_pad, int64_t *convstride, int64_t *dilation,
                   cudnnDataType_t dataType, at::Half *devPtrX,
                   at::Half *devPtrW, at::Half *devPtrB, at::Half *devPtrY) {
  ...
  try {
    ...
    auto cache_string =
        getConvFusionString(x_dim, conv_pad, convstride, dilation, w_dim,
                            dataType, opGraph.getTag());
...
void run_conv_bias_mask_relu(int64_t *x_dim, int64_t *w_dim, int64_t *y_dim,
                             int64_t *conv_pad, int64_t *conv_stride,
                             int64_t *conv_dilation, cudnnDataType_t dataType,
                             at::Half *devPtrX, at::Half *devPtrW,
                             at::Half *devPtrB, int8_t *devPtrM,
                             at::Half *devPtrY) {
  ...
  try {
    ...
    // Create an Operation Graph. In this case it is convolution bias activation
    std::array<cudnn_frontend::Operation const *, 4> ops = {&conv_op, &bias_op,
                                                            &mask_op, &act_op};
    ...
    // Create string encoding for plan caching
    auto cache_string =
        getConvFusionString(x_dim, conv_pad, conv_stride, conv_dilation, w_dim,
                            dataType, opGraph.getTag());
    DEBUG_CUDNN_MSG(log_buf, "[convstring] " << cache_string);
...
void run_conv_cscale_cbias_relu(int64_t *x_dim, int64_t *w_dim, int64_t *y_dim,
                                int64_t *conv_pad, int64_t *conv_stride,
                                int64_t *conv_dilation,
                                cudnnDataType_t dataType, at::Half *devPtrX,
                                at::Half *devPtrW, at::Half *devPtrS,
  ...
  try {
    ...
    // Create an Operation Graph. In this case it is convolution bias activation
    std::array<cudnn_frontend::Operation const *, 4> ops = {&conv_op, &scale_op,
                                                            &bias_op, &act_op};
...
void run_conv_bias_relu(int64_t *x_dim, int64_t *w_dim, int64_t *y_dim,
                        int64_t *conv_pad, int64_t *conv_stride,
                        int64_t *conv_dilation, cudnnDataType_t dataType,
                        at::Half *devPtrX, at::Half *devPtrW, at::Half *devPtrB,
                        at::Half *devPtrY) {
  ...
  try {
    ...
    // Create an Operation Graph. In this case it is convolution bias activation
    std::array<cudnn_frontend::Operation const *, 3> ops = {&conv_op, &bias_op,
                                                            &act_op};
...
void run_drelu_dscale(int64_t *dy_dim, cudnnDataType_t dataType,
                      at::Half *devPtrDY, at::Half *devPtrR, at::Half *devPtrS,
                      at::Half *devPtrDX) {
  ...
  try {
    ...
    auto cache_string =
        getConvFusionString(dy_dim, pad_dummy, stride_dummy, dilation_dummy,
                            s_dim, dataType, opGraph.getTag());
    DEBUG_CUDNN_MSG(log_buf, "[convstring] " << cache_string);
...
void run_drelu_dbias(int64_t *dy_dim, cudnnDataType_t dataType,
                     at::Half *devPtrDY, at::Half *devPtrR, at::Half *devPtrDR,
                     float *devPtrDB) {
...
void run_dconv_drelu_dbias(int64_t *x_dim, int64_t *w_dim, int64_t *y_dim,
                           int64_t *pad, int64_t *convstride, int64_t *dilation,
                           cudnnDataType_t dataType, at::Half *devPtrX,
                           at::Half *devPtrW, at::Half *devPtrR,
                           at::Half *devPtrRg, float *devPtrY) {
  cudnnHandle_t handle_ = torch::native::getCudnnHandle();
  std::stringstream log_buf;
  try {
    int convDim = 2;
    ...
    // Create string encoding for plan caching
    auto cache_string = getConvFusionString(x_dim, pad, convstride, dilation,
                                            w_dim, dataType, opGraph.getTag());
    DEBUG_CUDNN_MSG(log_buf, "[convstring] " << cache_string);
...
void run_dconv(int64_t *x_dim, int64_t *w_dim, int64_t *y_dim,
               int64_t *conv_pad, int64_t *conv_stride, int64_t *conv_dilation,
...
void run_dbias(int64_t *x_dim, cudnnDataType_t dataType, at::Half *devPtrX,
               float *devPtrY) {
  cudnnHandle_t handle_ = torch::native::getCudnnHandle();
  std::stringstream log_buf;
  try {
    ...
    auto cache_string =
        getConvFusionString(x_dim, pad_dummy, stride_dummy, dilation_dummy,
                            b_dim, dataType, opGraph.getTag());
    DEBUG_CUDNN_MSG(log_buf, "[convstring] " << cache_string);
...
conv_bias_mask_relu_forward(std::vector<at::Tensor> inputs, int64_t padding,
                            int64_t stride) {
  std::cout << std::fixed;
  ...
  for (int dim = 0; dim < 2; dim++) {
    y_dim[dim + 2] =
        getFwdConvOutputDim(x_dim[dim + 2], conv_pad[dim], w_dim[dim + 2],
                            conv_stride[dim], conv_dilation[dim]);
  }
  ...
  run_conv_bias_mask_relu(x_dim, w_dim, y_dim, conv_pad, conv_stride,
                          conv_dilation, CUDNN_DATA_HALF, x, w, b, m, y);

  DEBUG_MSG(
      "[DEBUG] conv-bias-mask-relu : " << y.to(at::kFloat).sum().item<float>());
...
at::Tensor conv_cscale_cbias_relu_forward(std::vector<at::Tensor> inputs,
  ...
  run_conv_cscale_cbias_relu(x_dim, w_dim, y_dim, conv_pad, conv_stride,
                             conv_dilation, CUDNN_DATA_HALF, x, w, s, b, y);

  DEBUG_MSG("[DEBUG] conv-cscale-cbias-relu : "
            << y.to(at::kFloat).sum().item<float>());

  return out;
}

std::vector<at::Tensor>
conv_cscale_cbias_relu_backward(std::vector<at::Tensor> inputs, int64_t padding,
                                int64_t stride) {
  ...
  run_dconv(x_dim, w_dim, y_dim, conv_pad, conv_stride, conv_dilation,
            CUDNN_DATA_HALF, x, dw, ds,
            CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR);
...
std::vector<at::Tensor> conv_bias_relu_forward(std::vector<at::Tensor> inputs,
                                               int64_t padding,
                                               int64_t stride) {
  std::cout << std::fixed;
  ...
  run_conv_bias_relu(x_dim, w_dim, y_dim, conv_pad, conv_stride, conv_dilation,
                     CUDNN_DATA_HALF, x, w, b, y);

  DEBUG_MSG(
      "[DEBUG] conv-bias-relu : " << y.to(at::kFloat).sum().item<float>());
...
std::vector<at::Tensor> conv_bias_relu_backward(std::vector<at::Tensor> inputs,
                                                int64_t padding,
                                                int64_t stride) {
  bool requires_grad = inputs[0].requires_grad();
...
std::vector<at::Tensor> conv_bias_forward(std::vector<at::Tensor> inputs,
                                          int64_t padding, int64_t stride) {
  std::cout << std::fixed;
  ...
  run_conv_bias(x_dim, w_dim, y_dim, conv_pad, conv_stride, conv_dilation,
                CUDNN_DATA_HALF, x, w, b, y);
...
std::vector<at::Tensor> conv_bias_backward(std::vector<at::Tensor> inputs,
                                           int64_t padding, int64_t stride) {
  bool requires_grad = inputs[0].requires_grad();
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("backward_no_relu", &conv_bias_backward, "Fused Conv-Bias backward",
        py::call_guard<py::gil_scoped_release>());
  m.def("forward_mask", &conv_bias_mask_relu_forward,
        "Fused Conv-Bias-Mask-ReLU forward",
        py::call_guard<py::gil_scoped_release>());
  m.def("forward_cscale_cbias_relu", &conv_cscale_cbias_relu_forward,
        "Fused Conv-(const)Scale-(const)Bias-ReLU",
        py::call_guard<py::gil_scoped_release>());
  m.def("backward_cscale_cbias_relu", &conv_cscale_cbias_relu_backward,
        "Fused Conv-(const)Scale-(const)Bias-ReLU backward",
        py::call_guard<py::gil_scoped_release>());
}
