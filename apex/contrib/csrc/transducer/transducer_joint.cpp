...
std::vector<torch::Tensor> transducer_joint_forward(
  ...
  return transducer_joint_cuda_forward(f, g, fLen, gLen, batchOffset,
                                       packedBatch, opt, packOutput, relu,
                                       dropout, dropoutProb, tileSize);
}

std::vector<torch::Tensor>
transducer_joint_backward(std::vector<torch::Tensor> in, torch::Tensor fLen,
                          torch::Tensor gLen, torch::Tensor batchOffset,
                          int maxFLen, int maxGLen, bool packOutput,
                          float scale) {
  for (auto t : in) {
    CHECK_INPUT(t);
  }
  ...
  if (packOutput)
    CHECK_INPUT(batchOffset);
  return transducer_joint_cuda_backward(in, fLen, gLen, batchOffset, maxFLen,
                                        maxGLen, packOutput, scale);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &transducer_joint_forward, "transducer joint forward (CUDA)",
        py::call_guard<py::gil_scoped_release>());
  m.def("backward", &transducer_joint_backward,
