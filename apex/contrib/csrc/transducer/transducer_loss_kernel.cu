...
__device__ __forceinline__ scalar_t logSumExp(scalar_t a, scalar_t b) {
  // standard log-sum-exp trick is used here to provide better numerical
  // stability
...
// Forward (alpha) and backward (beta) path are launched together. Input is
// assumed to be converted into log scale by the preceding log_softmax layer
// Diagonal wavefront advancing usually used in dynamic programming is leveraged
// here. alpha and beta are of acc_t type, as they are essentially accumulators.

// This loss function supports packed input where a tensor of shape [B, T, U, H]
// is packed into [B_packed, H]. Don't-care region (t > audLen) or (u > txtLen)
...
// transudcer loss function (i.e. forward-backward algorithm) with batch loading
// optimization. Compared to the vanilla version, there are two optimizations:
// 1. load x in batch through loop unrolling to reduce the latency.
// 2. Use registers and shared memory to hold alpha and beta values passed from
// one step the next. For simplicity, this kernel currently only supports U <=
// maxThread, which should be the common case. For cases where U > maxThread,
// the vanilla kernel is used as a fallback option.
...
// Detail of this loss function can be found in:
// [1] Sequence Transduction with Recurrent Neural Networks.
// For this backward kernel, bwd op for the preceding softmax is assumed to be
// handled elsewhere, hence only Eq(20) in [1] is implemented in this kernel.

// Each thread block works on [batch, t, :, :] of data. Each thread works on a
// specific u at a time Since only gradients for the correct token and null
// token need to be updated, gradients at other locations are initialized to 0.
...
// Each thread block works on [batch, t, u, :] of data. Each thread works on a
// specific h at a time
...
__global__ void transducer_loss_fused_backward(const scalar_t *x, const scalar_t *lossGrad, const int *audLen,
  ...
  if (t < myFLen and u < myGLen) {
    ...
    // In non-pack mode, need to make sure the gradients for don't-care regions
    // are zero.
    for (int64_t h = tid; h < dictSize; h += blockDim.x) {
      myXGrad[h] = 0;
    }
...
torch::Tensor transducer_loss_cuda_backward(torch::Tensor x, torch::Tensor lossGrad, torch::Tensor alpha,
  ...
  if (fuseSoftmaxBackward) {
    // alloc empty tensors for performance, hence need to ensure zeros are
    // writtern to don't-care region in the kernel.
