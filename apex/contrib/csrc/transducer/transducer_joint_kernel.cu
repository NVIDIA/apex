...
// Warp reduce kernels to reduce N groups of data into N numbers, where N =
...
// Helper class to calculate pointer offset that can be shared by different
// flavors of kernels For bwd, batch offset and stride are different for packing
// and non-packing mode. The reducion is done for two input tensors. Therefore,
...
// Each thread block is working on one "batch" of data in the output tensor,
// [batch, t, u, :]

// This joint function can optionally pack the output where the output tensor
// with a shape of [B, T, U, H] is packed into [B_packed, H]. Don't-care region
...
// Need to write finite data to don't-care region because we instantiate the
// result tensor with torch::empty for performance reasons. Even though it is
// don't-care region, the contents need to be finite, otherwise could lead to
// NaN in WGRAD. In packing mode, this write is no longer necessary as we remove
// the don't-care region from the output. Picking -1 (over 0) here for ease of
// testing.
#pragma unroll
    for (int h = threadIdx.x; h < hiddenSize; h += blockDim.x) {
      if (h < hiddenSize) {
...
sum = f.unsqueeze(dim=2) + g.unsqueeze(dim=1)
The resultant tensor is of shape [batch, T, U, H]
Each thread is working on a tile of the shape of tileF x tileG in the result
tensor. The input for the tile is first loaded in the register and is reused
tileG and tileF times.

This joint function can optionally pack the output where the output tensor with
a shape of [B, T, U, H] is packed into [B_packed, H]. Don't-care region (t >
fLen) or (u > gLen) is removed. To enable packing, the starting offset for each
batch need to be specified with batchOffset.

Optionally this joint function performs ReLU and/or dropout on the joint output,
which is controlled by arguments relu and dropout, respectively. philoxArgs is
argument used for generating pseudorandom number. When at least one of
operations in ReLU and dropout is activated, the joint function is a masked
...
/*
Bwd operation (reduction) on one input tensor. Since the operation performed for
the two input tensors are exactly the same, only one kernel is needed, and the
different indexing offsets and strides are handled by OffsetCalBwd.

When packing is enabled in the fwd op, unpacking is needed to restore the
gradients in a non-packed form.

When ReLU and/or dropout are performed in the fwd pass, this operation becomes a
...
The two bwd ops are launched together, the first op uses blockIdx.y < maxFLen,
and the second op uses the rest. When ReLU and/or dropout are performed in the
...
Doing exact same operation as transducer_joint_single_backward except the load
and store are vectorized. When packing is enabled in the fwd op, unpacking is
needed to restore the gradients in a non-packed form. When ReLU and/or dropout
...
std::vector<torch::Tensor> transducer_joint_cuda_forward(torch::Tensor f, torch::Tensor g, torch::Tensor fLen,
  ...
  if (opt == 1) {
    // tiled version. For simplicity, assume tileF == tileG, even though the
    // kernel can support more general cases.
    const int threads = numThread;
...
std::vector<torch::Tensor> transducer_joint_cuda_backward(std::vector<torch::Tensor> in, torch::Tensor fLen,
  ...
  // Need smem for transposing the partial sum. The partial sum is in a matrix
  // of the shape numWarp x warpSize
