...
struct DistOptLAMBStage1Functor {
  ...
  operator()(int chunk_size, volatile int *noop_gmem, TensorListMetadata<5> &tl,
             ...
             const int *per_tensor_bias_correction, const int *step,
             const MATH_T *per_tensor_epsilon, adamMode_t mode,
    ...
    if (max_grad_norm > 0) {
      combined_scale =
          max_grad_norm / (*global_grad_norm / *global_scale + 1e-6);
...
struct DistOptLAMBStage2Functor {
  ...
  operator()(int chunk_size, volatile int *noop_gmem, TensorListMetadata<3> &tl,
    ...
    if (use_nvlamb || (decay != (MATH_T)0.0)) {
      ...
      MATH_T update_norm =
          per_tensor_update_norm[update_norm_offset[tensor_num]];
      ratio = (update_norm != 0.0 && param_norm != 0.0)
      ...
      DISPATCH_FLOAT_AND_HALF(
          ...
          DISPATCH_FLOAT_AND_HALF(
              ...
              multi_tensor_apply<5>(
                  ...
                  DistOptLAMBStage1Functor<scalar_t_0, scalar_t_1,
                                           scalar_t_2>(),
                  ...
                  step.data_ptr<int>(),
                  per_tensor_epsilon.data_ptr<scalar_t_2>(), (adamMode_t)mode,
      ...
      DISPATCH_FLOAT_HALF_AND_BYTE(
          ...
          DISPATCH_FLOAT_AND_HALF(
              ...
              multi_tensor_apply<3>(
                  BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                  DistOptLAMBStage2Functor<scalar_t_0, scalar_t_1,
                                           scalar_t_2>(),
