...
void fused_adam_cuda_mt(int chunk_size, at::Tensor overflow_flag,
...
// C++ interface
void strided_check_finite(at::Tensor &overflow_flag, at::Tensor &p_copy,
                          int stride, int clear_overflow_first) {
  CHECK_INPUT(p_copy);
  fused_strided_check_finite(overflow_flag, p_copy, stride,
...
void reversible_adam(at::Tensor &p, at::Tensor &p_copy, at::Tensor &m,
                     at::Tensor &v, at::Tensor &g, float lr, float beta1,
                     float beta2, float eps, float grad_scale, int step,
                     int mode, int bias_correction, float decay) {
  CHECK_INPUT(p);
  ...
  fused_reversible_adam_cuda(p, p_copy, m, v, g, lr, beta1, beta2, eps,
                             grad_scale, step, mode, bias_correction, decay);
}
void maybe_adam_undo(at::Tensor &overflow_flag, at::Tensor &p, at::Tensor &m,
                     at::Tensor &v, at::Tensor &g, float lr, float beta1,
                     float beta2, float eps, float grad_scale, int step,
                     int mode, int bias_correction, float decay) {
  CHECK_INPUT(p);
  ...
  fused_maybe_adam_undo_cuda(overflow_flag, p, m, v, g, lr, beta1, beta2, eps,
                             grad_scale, step, mode, bias_correction, decay);
}
void maybe_cast(at::Tensor &overflow_flag, at::Tensor &p_in,
                at::Tensor &p_out) {
  CHECK_INPUT(p_in);
  CHECK_INPUT(p_out);
  int64_t num_elem = p_in.numel();
  TORCH_CHECK(p_out.numel() == num_elem,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("adam", &adam, "Adam optimized CUDA implementation.",
        py::call_guard<py::gil_scoped_release>());
  m.def("reversible_adam", &reversible_adam,
        "Reversible Adam optimized CUDA implementation.",
        py::call_guard<py::gil_scoped_release>());
  m.def("adam_mt", &fused_adam_cuda_mt,
        "Multi tensor Adam optimized CUDA implementation.",
        py::call_guard<py::gil_scoped_release>());
  m.def("maybe_adam_undo", &maybe_adam_undo,
  ...
  m.def("maybe_cast", &maybe_cast, "Unpack byte tensor containing e5m2 floats.",
        py::call_guard<py::gil_scoped_release>());
  m.def("maybe_cast_mt", &maybe_cast_cuda_mt,
