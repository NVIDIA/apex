...
void multi_tensor_fused_adam_dispatch(int64_t chunk_size, at::Tensor noop_flag,
                                      std::vector<std::vector<at::Tensor>> tensor_lists, at::Tensor grad_scale,
                                      double lr, double beta1, double beta2, double eps, int64_t step, int64_t mode,
                                      int64_t bias_correction, double weight_decay) {
  multi_tensor_fused_adam_cuda(static_cast<int>(chunk_size), noop_flag, tensor_lists, grad_scale, static_cast<float>(lr),
...
void multi_tensor_fused_adam_capturable_dispatch(int64_t chunk_size, at::Tensor noop_flag,
                                                 std::vector<std::vector<at::Tensor>> tensor_lists,
                                                 at::Tensor grad_scale, at::Tensor lr, double beta1, double beta2,
                                                 double eps, at::Tensor step, int64_t mode, int64_t bias_correction,
                                                 double weight_decay) {
  multi_tensor_fused_adam_capturable_cuda(static_cast<int>(chunk_size), noop_flag, tensor_lists, grad_scale, lr,
                                          ...
                                          static_cast<float>(eps), step, static_cast<int>(mode),
                                          static_cast<int>(bias_correction), static_cast<float>(weight_decay));
}

void multi_tensor_fused_adam_with_param_remainders_dispatch(int64_t chunk_size, at::Tensor noop_flag,
                                                            std::vector<std::vector<at::Tensor>> tensor_lists,
                                                            at::Tensor grad_scale, double lr, double beta1,
                                                            double beta2, double eps, int64_t step, int64_t mode,
                                                            int64_t bias_correction, double weight_decay) {
...
TORCH_LIBRARY_FRAGMENT(apex, m) {
  m.def("distributed_adam_multi_tensor_fused_adam(int chunk_size, Tensor noop_flag, Tensor[][] tensor_lists, "
        "Tensor grad_scale, float lr, float beta1, float beta2, float eps, int step, int mode, "
        "int bias_correction, float weight_decay) -> ()");
  m.def("distributed_adam_multi_tensor_fused_adam_capturable(int chunk_size, Tensor noop_flag, "
  ...
  m.def("distributed_adam_multi_tensor_fused_adam_with_param_remainders(int chunk_size, Tensor noop_flag, "
...
TORCH_LIBRARY_IMPL(apex, CUDA, m) {
  m.impl("distributed_adam_multi_tensor_fused_adam", &multi_tensor_fused_adam_dispatch);
  m.impl("distributed_adam_multi_tensor_fused_adam_capturable", &multi_tensor_fused_adam_capturable_dispatch);
  m.impl("distributed_adam_multi_tensor_fused_adam_with_param_remainders",
         &multi_tensor_fused_adam_with_param_remainders_dispatch);
}
