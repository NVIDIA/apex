...
#define DISPATCH_NUM_GROUPS_AND_SILU(num_groups, silu, NUM_GROUPS, SILU, ...)  \
  [&] {                                                                        \
    ...
    throw std::invalid_argument("DISPATCH_NUM_GROUPS_AND_SILU " +              \
...
template <typename T> void gn_cuda(GN_CUDA_HOST_PARAMS(T)) {
...
template <typename T> void gn_bwd_cuda(GN_BWD_CUDA_HOST_PARAMS(T)) {
  DISPATCH_HW_C(hw, num_groups * channels_per_group, HW, C, [&] {
    DISPATCH_NUM_GROUPS_AND_SILU(num_groups, silu, G, SILU, [&] {
