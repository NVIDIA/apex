...
#define DISPATCH_LOWER_BOUND_N(VALUE, CONST_NAME, ...)                         \
  [&] {                                                                        \
    ...
    throw std::invalid_argument("DISPATCH_LOWER_BOUND_N " +                    \
                                std::to_string(VALUE));                        \
  }()

#define DISPATCH_CUDA_ARCH_AND_LOWER_BOUND_SM_COUNT(                           \
    runtime_cuda_arch, sm_count, RUNTIME_CUDA_ARCH, LB_SM_COUNT, ...)          \
  [&] {                                                                        \
    ...
    throw std::invalid_argument(                                               \
        "DISPATCH_CUDA_ARCH_AND_LOWER_BOUND_SM_COUNT " +                       \
        std::to_string(runtime_cuda_arch) + " " + std::to_string(sm_count));   \
  }()
...
#define DISPATCH_SM_MARGIN(VALUE, CONST_NAME, ...)                             \
  [&] {                                                                        \
    ...
    throw std::invalid_argument("DISPATCH_SM_MARGIN " +                        \
                                std::to_string(VALUE));                        \
  }()
...
template <typename T, bool BWD, bool REQUIRES_WGRAD, int HW, int G, int CPG,
          int LB_N, int RUNTIME_CUDA_ARCH, int LB_SM_COUNT,
          int EFFECTIVE_CUDA_ARCH, int SM_MARGIN>
constexpr auto compute_gn_params() {
  ...
  //   - block sync: each block handles a whole group, i.e., a multiple of (G *
  //   HW) elements
  //   - virtual cluster sync: each virtual cluster handles a group
  // Block sync can avoid cross-block synchronization latency, but it may cause
  // low occupancy.
  //   Use block sync if the IO size is small, when latency rather than
  //   occupancy dominates the kernel running time.

  // Elements to load for forward pass is `x`, elements to load for backward
  // pass are `x` and `grad_output`, hence there is a factor of (1 + BWD)
  if (HW * CPG * (1 + BWD) * sizeof(T) <= 20480) {
    ...
    //   We have to leave some room for other variables and compiler
    //   optimizations, so we use 36000 as the threshold.
    LOAD_TWICE =
        BLOCKS_PER_SM * ROWS_PER_BLOCK * C_PER_BLOCK * (1 + BWD) * sizeof(T) >
        36000 * 4;
  } else {
    // Strategy 2: virtual cluster sync
    //   A virtual cluster is a group of blocks that are synchronized with each
    //   other. Each group, i.e., a multiple of (G * HW) elements, should be
    //   handled on the same virtual cluster. If the virtual cluster size is
    //   supported by the hardware, HARDWARE_CLUSTER is preferred; otherwise,
    //   cooperative groups are used (i.e., PERSISTENT kernels).
    ...
    for (int blocks_per_sm = 1; blocks_per_sm <= 3; blocks_per_sm++) {
      for (int rows_per_block = HW; rows_per_block >= 1; rows_per_block /= 2) {
        int virtual_cluster_size =
            (HW / rows_per_block) * (c_per_cluster / C_PER_BLOCK);
        if (virtual_cluster_size > blocks_per_sm * (LB_SM_COUNT - SM_MARGIN)) {
          continue;
        }
        int num_clusters =
            blocks_per_sm * (LB_SM_COUNT - SM_MARGIN) / virtual_cluster_size;
        ...
        int wave_util = 10000 * std::min(num_tasks, num_clusters) *
        ...
        decltype(best_candidate) candidate = {
            ...
            !(num_waves >= 2 &&
              blocks_per_sm ==
                  1),   // When there are multiple waves, prefer multiple blocks
                        // per SM to ensure overlapping
            -num_waves, // Prefer fewer waves
            std::min(9000, wave_util), // Prefer high wave utilization
            -blocks_per_sm, // Prefer fewer blocks per SM in order to reduce
                            // threads overhead
        };
        if (candidate > best_candidate) {
          ...
          static_assert(std::tuple_size<decltype(best_candidate)>::value == 6,
                        "missing assignments");
  ...
  //   For example, on H100, when gridDim=128 and each block occupies the whole
  //   SM,
  //     if cluster is not used, all blocks can be active simultaneously.
  //     if cluster size is 16, not all blocks can be active simultaneously
  //     (which can be queried by cudaOccupancyMaxActiveClusters),
  //       so there will be two waves which impacts efficiency.
  // When SM_MARGIN is set, no cluster should be used because other kernels may
  // occupy a part of the cluster.
  bool HARDWARE_CLUSTER =
      virtual_cluster_size <= 2 && virtual_cluster_size != 1 && SM_MARGIN == 0;

  int MAX_VEC_BYTES = 8; // Sometimes 4 or 16 is better, but there is no trivial
                         // way to select the best vectorization size.
  int VEC_ELEMS = std::min(gcd(MAX_VEC_BYTES / (int)sizeof(T), C_PER_BLOCK),
                           gcd(MAX_VEC_BYTES / (int)sizeof(T),
                               ROWS_PER_BLOCK * C_PER_BLOCK / BLOCK_DIM_X));

  return std::make_tuple(BLOCK_DIM_X, C_PER_BLOCK, ROWS_PER_BLOCK, VEC_ELEMS,
                         LOAD_TWICE, BLOCKS_PER_SM, HARDWARE_CLUSTER,
                         wgrad_sync_method);
}

// Save compilation time for unused CUDA_ARCHs
//   For each template argument from
//   DISPATCH_CUDA_ARCH_AND_LOWER_BOUND_SM_COUNT, the kernel is only compiled
//   for the corresponding CUDA_ARCH
template <int EFFECTIVE_CUDA_ARCH> class CompileCondition {
public:
        ...
        runtime_cuda_arch, sm_count, RUNTIME_CUDA_ARCH, LB_SM_COUNT, [&] {
          DISPATCH_SM_MARGIN(sm_margin, SM_MARGIN, [&] {
            ...
            constexpr int EFFECTIVE_CUDA_ARCH =
                std::min(RUNTIME_CUDA_ARCH,
                         get_max_cuda_arch()); // Assume the max CUDA_ARCH is
                                               // used to generate PTX

            constexpr int CPG = C / G;

            constexpr auto params =
                compute_gn_params<T, false, false, HW, G, CPG, LB_N,
                                  RUNTIME_CUDA_ARCH, LB_SM_COUNT,
            ...
            constexpr int NUM_VIRTUAL_CLUSTERS =
                ((LB_SM_COUNT - SM_MARGIN) * BLOCKS_PER_SM) /
                VIRTUAL_CLUSTER_SIZE;
            constexpr bool PERSISTENT =
                !HARDWARE_CLUSTER &&
                VIRTUAL_CLUSTER_SIZE >=
                    2; // Only virtual cluster sync (not include hardware
                       // cluster sync) requires PERSISTENT kernels
            ...
            if (meta_ptr) {
              ...
              meta_ptr->red_buffer_size = 2 * NUM_VIRTUAL_CLUSTERS *
                                          VIRTUAL_CLUSTER_SIZE *
                                          MAX_NUM_GROUPS_PER_BLOCK * 2;
            ...
            config.gridDim =
                dim3(VIRTUAL_CLUSTER_SIZE,
                     PERSISTENT ? std::min((int)n * (C / C_PER_CLUSTER),
            ...
            if constexpr (HARDWARE_CLUSTER) {
              ...
              attribute[0].val.clusterDim.x =
                  VIRTUAL_CLUSTER_SIZE; // Cluster size in X-dimension
            ...
            auto kernel =
                &gn_cuda_kernel<T, BLOCK_DIM_X, BLOCKS_PER_SM, G, CPG, HW, SILU,
                                ROWS_PER_BLOCK, C_PER_BLOCK, C_PER_CLUSTER,
                                VEC_ELEMS, PERSISTENT, NUM_VIRTUAL_CLUSTERS,
                                LOAD_TWICE, HARDWARE_CLUSTER,
                                CompileCondition<EFFECTIVE_CUDA_ARCH>>;
            if constexpr (HARDWARE_CLUSTER) {
              ...
              CUDA_CHECK(cudaOccupancyMaxPotentialClusterSize(
                  &max_cluster_size, (void *)kernel, &config));
              if (VIRTUAL_CLUSTER_SIZE <= max_cluster_size && PERSISTENT) {
                ...
                CUDA_CHECK(cudaOccupancyMaxActiveClusters(
                    &active_clusters, (void *)kernel, &config));
              }
              if (VIRTUAL_CLUSTER_SIZE <= max_cluster_size &&
                  (!PERSISTENT ||
                   PERSISTENT && NUM_VIRTUAL_CLUSTERS <= active_clusters)) {
                attribute[0].val.clusterDim.x = VIRTUAL_CLUSTER_SIZE;
              } else {
                // Fallback to cooperative groups because hardware cluster
                // cannot be active simultaneously
                constexpr bool HARDWARE_CLUSTER_NEW = false;
                constexpr bool PERSISTENT_NEW =
                    !HARDWARE_CLUSTER_NEW && VIRTUAL_CLUSTER_SIZE >= 2;
                config.gridDim =
                    dim3(VIRTUAL_CLUSTER_SIZE,
                         PERSISTENT_NEW ? std::min((int)n * (C / C_PER_CLUSTER),
                                                   NUM_VIRTUAL_CLUSTERS)
                                        : n * (C / C_PER_CLUSTER),
                         1);
                ...
                if constexpr (PERSISTENT_NEW) {
                  attribute[config.numAttrs].id =
                      cudaLaunchAttributeCooperative;
                  attribute[config.numAttrs].val.cooperative = 1;
                  config.attrs = attribute;
                  config.numAttrs++;
                }
                kernel = &gn_cuda_kernel<T, BLOCK_DIM_X, BLOCKS_PER_SM, G, CPG,
                                         HW, SILU, ROWS_PER_BLOCK, C_PER_BLOCK,
                                         C_PER_CLUSTER, VEC_ELEMS,
                                         PERSISTENT_NEW, NUM_VIRTUAL_CLUSTERS,
                                         LOAD_TWICE, HARDWARE_CLUSTER_NEW,
                                         CompileCondition<EFFECTIVE_CUDA_ARCH>>;
              }
            }
            CUDA_CHECK(cudaLaunchKernelEx(&config, kernel, out, x, w, b, eps, n,
                                          mean_var_out, red_buffer, barrier));
          });
        });
  });
}
    ...
    DISPATCH_CUDA_ARCH_AND_LOWER_BOUND_SM_COUNT(
        runtime_cuda_arch, sm_count, RUNTIME_CUDA_ARCH, LB_SM_COUNT, [&] {
          DISPATCH_SM_MARGIN(sm_margin, SM_MARGIN, [&] {
            ...
            constexpr auto params =
                compute_gn_params<T, true, REQUIRES_WGRAD, HW, G, CPG, LB_N,
                                  RUNTIME_CUDA_ARCH, LB_SM_COUNT,
                                  EFFECTIVE_CUDA_ARCH, SM_MARGIN>();
            constexpr int BLOCK_DIM_X = std::get<0>(params);
            constexpr int C_PER_BLOCK = std::get<1>(params);
            ...
            constexpr int BLOCKS_PER_SM = std::get<5>(params);
            constexpr bool HARDWARE_CLUSTER = std::get<6>(params);
            constexpr WgradSyncMethod wgrad_sync_method_hint =
                std::get<7>(params);
            ...
            // PERSISTENT is required because wgrad reduction requires
            // synchronization.
            //   TODO: specilize for the case that REQUIRES_WGRAD == false
            constexpr bool PERSISTENT = true;

            // Determine whether to align each virtual cluster to a fixed range
            // of channels
            //   If aligned, WGRAD_REUSE_SUM_SYNC_GROUP can be used, then less
            //   local wgrad memory is used (leave more room for compiler
            //     optimizations), and wgrad reduction is more efficient.
            //   However, aligning can cause low occupancy.
            //   There is a trade-off, and the condition to align is
                ...
                wgrad_sync_method_hint == WGRAD_SYNC_UNSPECIFIED
                    ? NUM_VIRTUAL_CLUSTERS_NOT_ALIGNED >
                              ...
                              NUM_VIRTUAL_CLUSTERS_NOT_ALIGNED %
                                      (C / C_PER_CLUSTER) ==
                                  0
                          ? (HARDWARE_CLUSTER ? WGRAD_ARRIVE_AND_WAIT_GROUP
            ...
            if (meta_ptr) {
              ...
              meta_ptr->red_buffer_size =
                  2 * NUM_VIRTUAL_CLUSTERS * VIRTUAL_CLUSTER_SIZE *
                      MAX_NUM_GROUPS_PER_BLOCK * 2 +
                  std::max(n, (int64_t)NUM_VIRTUAL_CLUSTERS /
                                  (C / C_PER_CLUSTER)) *
                      (HW / ROWS_PER_BLOCK) * C * 2;
            ...
            config.gridDim = dim3(
                VIRTUAL_CLUSTER_SIZE,
                PERSISTENT ? NUM_VIRTUAL_CLUSTERS : n * (C / C_PER_CLUSTER), 1);
            config.blockDim = BLOCK_DIM_X;
            config.stream = stream;
            ...
            auto kernel = &gn_bwd_cuda_kernel<
                T, BLOCK_DIM_X, BLOCKS_PER_SM, G, CPG, HW, SILU, REQUIRES_WGRAD,
                ROWS_PER_BLOCK, C_PER_BLOCK, C_PER_CLUSTER, VEC_ELEMS,
                PERSISTENT, NUM_VIRTUAL_CLUSTERS, LOAD_TWICE, HARDWARE_CLUSTER,
                wgrad_sync_method, CompileCondition<EFFECTIVE_CUDA_ARCH>>;
            if constexpr (HARDWARE_CLUSTER) {
              if constexpr (VIRTUAL_CLUSTER_SIZE > 8) {
                CUDA_CHECK(cudaFuncSetAttribute(
                    kernel, cudaFuncAttributeNonPortableClusterSizeAllowed, 1));
              ...
              if (VIRTUAL_CLUSTER_SIZE <= max_cluster_size &&
                ...
                // Fallback to cooperative groups for dgrad computation because
                // hardware cluster cannot be active simultaneously
                attribute[0].val.clusterDim.x = 1;
                kernel = &gn_bwd_cuda_kernel<
                    T, BLOCK_DIM_X, BLOCKS_PER_SM, G, CPG, HW, SILU,
                    REQUIRES_WGRAD, ROWS_PER_BLOCK, C_PER_BLOCK, C_PER_CLUSTER,
                    VEC_ELEMS, PERSISTENT, NUM_VIRTUAL_CLUSTERS, LOAD_TWICE,
                    false, wgrad_sync_method,
                    CompileCondition<EFFECTIVE_CUDA_ARCH>>;
              }
            }
            CUDA_CHECK(cudaLaunchKernelEx(
                &config, kernel, grad_input, grad_weight, grad_bias,
                grad_output, x, w, b, mean_var, eps, n, red_buffer, barrier));
          });
        });
  });
}
...
#define GN_CUDA_INST_DEFINE(HW, C)                                             \
  template void gn_cuda_single_shape<half, HW, C, 16, true>(                   \
      GN_CUDA_HOST_PARAMS(half));                                              \
  template void gn_cuda_single_shape<half, HW, C, 32, false>(                  \
      GN_CUDA_HOST_PARAMS(half));                                              \
  template void gn_bwd_cuda_single_shape<half, HW, C, 16, true>(               \
      GN_BWD_CUDA_HOST_PARAMS(half));                                          \
  template void gn_bwd_cuda_single_shape<half, HW, C, 32, false>(              \
      GN_BWD_CUDA_HOST_PARAMS(half));                                          \
  template void gn_cuda_single_shape<__nv_bfloat16, HW, C, 16, true>(          \
      GN_CUDA_HOST_PARAMS(__nv_bfloat16));                                     \
  template void gn_cuda_single_shape<__nv_bfloat16, HW, C, 32, false>(         \
      GN_CUDA_HOST_PARAMS(__nv_bfloat16));                                     \
  template void gn_bwd_cuda_single_shape<__nv_bfloat16, HW, C, 16, true>(      \
      GN_BWD_CUDA_HOST_PARAMS(__nv_bfloat16));                                 \
  template void gn_bwd_cuda_single_shape<__nv_bfloat16, HW, C, 32, false>(     \
      GN_BWD_CUDA_HOST_PARAMS(__nv_bfloat16));
