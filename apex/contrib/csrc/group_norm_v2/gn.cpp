...
torch::Tensor gn(torch::Tensor x, torch::Tensor w, torch::Tensor b, float eps, bool silu, int num_groups,
  ...
  if (x.dtype() == torch::kHalf && w.dtype() == torch::kHalf) {
    group_norm_v2::gn_cuda((half *)out.data_ptr(), (half *)x.data_ptr(), (half *)w.data_ptr(), (half *)b.data_ptr(),
                           eps, silu, x.size(0), x.size(2) * x.size(3), num_groups, x.size(1) / num_groups,
                           ptr_mean_var_out, nullptr, nullptr, sm_margin, stream, device_id, &meta, true);
  } else if (x.dtype() == torch::kBFloat16 && w.dtype() == torch::kBFloat16) {
    group_norm_v2::gn_cuda((__nv_bfloat16 *)out.data_ptr(), (__nv_bfloat16 *)x.data_ptr(),
                           (__nv_bfloat16 *)w.data_ptr(), (__nv_bfloat16 *)b.data_ptr(), eps, silu, x.size(0),
                           x.size(2) * x.size(3), num_groups, x.size(1) / num_groups, ptr_mean_var_out, nullptr,
  ...
  if (x.dtype() == torch::kHalf && w.dtype() == torch::kHalf) {
    group_norm_v2::gn_cuda((half *)out.data_ptr(), (half *)x.data_ptr(), (half *)w.data_ptr(), (half *)b.data_ptr(),
                           eps, silu, x.size(0), x.size(2) * x.size(3), num_groups, x.size(1) / num_groups,
                           ptr_mean_var_out, red_buffer.data_ptr<float>(), barrier.data_ptr<unsigned>(), sm_margin,
                           stream, device_id, nullptr, false);
  } else if (x.dtype() == torch::kBFloat16 && w.dtype() == torch::kBFloat16) {
    group_norm_v2::gn_cuda((__nv_bfloat16 *)out.data_ptr(), (__nv_bfloat16 *)x.data_ptr(),
                           (__nv_bfloat16 *)w.data_ptr(), (__nv_bfloat16 *)b.data_ptr(), eps, silu, x.size(0),
                           x.size(2) * x.size(3), num_groups, x.size(1) / num_groups, ptr_mean_var_out,
...
auto gn_bwd(torch::Tensor grad_output, torch::Tensor x, torch::Tensor w, torch::Tensor b, torch::Tensor mean_var,
  ...
  group_norm_v2::Meta meta;
  if (x.dtype() == torch::kHalf && w.dtype() == torch::kHalf) {
    group_norm_v2::gn_bwd_cuda((half *)grad_input.data_ptr(), (half *)grad_weight.data_ptr(),
                               (half *)grad_bias.data_ptr(), (half *)grad_output.data_ptr(), (half *)x.data_ptr(),
                               (half *)w.data_ptr(), (half *)b.data_ptr(), mean_var.data_ptr<float>(), eps, silu,
                               x.size(0), x.size(2) * x.size(3), num_groups, x.size(1) / num_groups, nullptr, nullptr,
                               sm_margin, stream, device_id, &meta, true);
  } else if (x.dtype() == torch::kBFloat16 && w.dtype() == torch::kBFloat16) {
    group_norm_v2::gn_bwd_cuda((__nv_bfloat16 *)grad_input.data_ptr(), (__nv_bfloat16 *)grad_weight.data_ptr(),
                               ...
                               (__nv_bfloat16 *)x.data_ptr(), (__nv_bfloat16 *)w.data_ptr(),
                               (__nv_bfloat16 *)b.data_ptr(), mean_var.data_ptr<float>(), eps, silu, x.size(0),
  ...
  if (x.dtype() == torch::kHalf && w.dtype() == torch::kHalf) {
    group_norm_v2::gn_bwd_cuda(
        ...
        (half *)grad_output.data_ptr(), (half *)x.data_ptr(), (half *)w.data_ptr(), (half *)b.data_ptr(),
        mean_var.data_ptr<float>(), eps, silu, x.size(0), x.size(2) * x.size(3), num_groups, x.size(1) / num_groups,
