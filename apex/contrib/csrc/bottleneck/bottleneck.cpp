...
int checkCudnnError(cudnnStatus_t code, const char *expr, const char *file,
                    int line) {
  if (code) {
...
void checkError(cudaError_t code, char const *func, const char *file,
                const int line, bool abort = true);
#define checkCUDAError(val)                                                    \
  { checkError((val), #val, __FILE__, __LINE__); } // in-line regular function

void checkError(cudaError_t code, char const *func, const char *file,
                const int line, bool abort) {
  if (code != cudaSuccess) {
    const char *errorMessage = cudaGetErrorString(code);
    fprintf(stderr,
            "CUDA error returned from \"%s\" at %s:%d, Error code: %d (%s)\n",
...
int getFwdConvOutputDim(int tensorDim, int pad, int filterDim, int stride,
                        int dilation) {
...
using common_conv_descriptors =
    std::tuple<cudnn_frontend::Tensor, cudnn_frontend::Tensor,
               cudnn_frontend::Tensor, cudnn_frontend::ConvDesc>;

common_conv_descriptors create_common_descriptors(
    int64_t *x_dim_padded, int64_t *padA, int64_t *convstrideA,
...
dconv_descriptors create_dconv_descriptors(int64_t *x_dim_padded, int64_t *padA,
                                           int64_t *convstrideA,
                                           int64_t *dilationA,
...
// TODO: better name
std::string getConvFusionString(int64_t *x_dim_padded, int64_t *padA,
                                int64_t *convstrideA, int64_t *dilationA,
                                int64_t *w_dim_padded, cudnnDataType_t dataType,
...
getOrCreatePlan(cudnnHandle_t handle_, std::stringstream &log_buf,
                cudnn_frontend::OperationGraph &opGraph,
                std::string cache_string, bool use_heuristic = true) {
  ...
  if (it != plan_cache.end()) {
    ...
    if (use_heuristic) {
      ...
      DEBUG_CUDNN_MSG(log_buf, opGraph.describe()
                                   << " has " << total_engines << " engines.");
      ...
      plan_cache.emplace(cache_string,
                         std::move(cudnn_frontend::ExecutionPlanBuilder()
                                       .setHandle(handle_)
...
void run_conv_scale_bias_add_activation(
    int64_t *x_dim_padded, int64_t *pad, int64_t *convstride, int64_t *dilation,
  ...
  try {
    ...
    common_convbias_descriptors tensors = create_conv_bias_add_act_descriptors(
        x_dim_padded, pad, convstride, dilation, w_dim_padded, y_dim_padded,
        dataType);
    ...
    auto act_op = cudnn_frontend::OperationBuilder(
                      CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
                      .setxDesc(devPtrI ? add_op.getOutputTensor()
                                        : bias_op.getOutputTensor())
                      .setyDesc(std::get<Y_TENSOR>(tensors))
                      .setpwDesc(actDesc)
                      .build();
    DEBUG_CUDNN_MSG(log_buf, act_op.describe());

    // Create an Operation Graph. In this case it is convolution add bias
    // activation
    std::array<cudnn_frontend::Operation const *, 5> ops = {
        &conv_op, &scale_op, &bias_op, devPtrI ? &add_op : &act_op, &act_op};
    ...
    auto cache_string =
        getConvFusionString(x_dim_padded, pad, convstride, dilation,
                            w_dim_padded, dataType, opGraph.getTag());
    ...
    DEBUG_CUDNN_MSG(log_buf, plan.describe()
                                 << " requires workspace " << workspace_size);
...
void run_conv_scale_bias(int64_t *x_dim_padded, int64_t *pad,
                         int64_t *convstride, int64_t *dilation,
                         int64_t *w_dim_padded, int64_t *y_dim_padded,
                         cudnnDataType_t dataType, at::Half *devPtrX,
                         at::Half *devPtrW, at::Half *devPtrY,
                         at::Half *devPtrZ, at::Half *devPtrB) {
  cudnnHandle_t handle_ = torch::native::getCudnnHandle();
  std::stringstream log_buf;
  try {
    int convDim = 2;
    ...
    auto scale_op = cudnn_frontend::OperationBuilder(
                        ...
                        .setxDesc(conv_op.getOutputTensor())
                        .setbDesc(std::get<Z_TENSOR>(tensors))
                        .setyDesc(std::get<AFTERADD_TENSOR>(
                            tensors)) // TODO: change enum to aftermul
                        .setpwDesc(scaleDesc)
                        .build();
    DEBUG_CUDNN_MSG(log_buf, scale_op.describe());
    ...
    // Create an Operation Graph. In this case it is convolution add bias
    // activation
    std::array<cudnn_frontend::Operation const *, 3> ops = {&conv_op, &scale_op,
                                                            &add_op};
...
void run_dconv_drelu_dscale(int64_t *x_dim_padded, int64_t *pad,
                            int64_t *convstride, int64_t *dilation,
                            int64_t *w_dim_padded, int64_t *y_dim_padded,
                            cudnnDataType_t dataType, at::Half *devPtrX,
                            at::Half *devPtrW, at::Half *devPtrY,
                            at::Half *devPtrZ, at::Half *devPtrR) {
  cudnnHandle_t handle_ = torch::native::getCudnnHandle();
  std::stringstream log_buf;
  try {
    int convDim = 2;
    ...
    dconv_descriptors tensors =
        create_dconv_descriptors(x_dim_padded, pad, convstride, dilation,
    ...
    // Create an Operation Graph. In this case it is convolution add bias
    // activation
    std::array<cudnn_frontend::Operation const *, 3> ops = {&conv_op, &act_op,
                                                            &scale_op};
...
void run_dconv(int64_t *x_dim_padded, int64_t *pad, int64_t *convstride,
               int64_t *dilation, int64_t *w_dim_padded, int64_t *y_dim_padded,
               cudnnDataType_t dataType, at::Half *devPtrX, at::Half *devPtrW,
...
void run_dconv_add(int64_t *x_dim_padded, int64_t *pad, int64_t *convstride,
                   int64_t *dilation, int64_t *w_dim_padded,
                   int64_t *y_dim_padded, cudnnDataType_t dataType,
                   at::Half *devPtrX, at::Half *devPtrW, at::Half *devPtrY,
                   at::Half *devPtrR) {
  cudnnHandle_t handle_ = torch::native::getCudnnHandle();
  std::stringstream log_buf;
  try {
...
std::vector<at::Tensor> bottleneck_forward(bool explicit_nhwc, int stride_1X1,
                                           std::vector<at::Tensor> inputs) {

  std::cout << std::fixed;
  // create output vector
  std::vector<at::Tensor> outputs;
  auto output_format = explicit_nhwc ? at::MemoryFormat::Contiguous
  ...
  for (int dim = 0; dim < 2; dim++) {
    outdimA1[dim + 2] =
        getFwdConvOutputDim(dimA[dim + 2], padA[dim], filterdimA1[dim + 2],
                            convstride1X1[dim], dilationA[dim]);
  }
  ...
  for (int dim = 0; dim < 2; dim++) {
    outdimA2[dim + 2] =
        getFwdConvOutputDim(outdimA1[dim + 2], padA1[dim], filterdimA2[dim + 2],
  ...
  for (int dim = 0; dim < 2; dim++) {
    outdimA3[dim + 2] =
        getFwdConvOutputDim(outdimA2[dim + 2], padA[dim], filterdimA3[dim + 2],
                            convstrideA[dim], dilationA[dim]);
  }
  ...
  run_conv_scale_bias_add_activation(dimA, padA, convstride1X1, dilationA,
                                     filterdimA1, outdimA1, CUDNN_DATA_HALF, x,
                                     w, y1, z, b, nullptr);
  ...
  run_conv_scale_bias_add_activation(outdimA1, padA1, convstrideA, dilationA,
                                     filterdimA2, outdimA2, CUDNN_DATA_HALF, y1,
                                     w, y2, z, b, nullptr);
  ...
  if (stride_1X1 != 1 || filterdimA3[0] != dimA[1]) {
    ...
    run_conv_scale_bias(dimA, padA, convstride1X1, dilationA, filterdimA4,
                        outdimA3, CUDNN_DATA_HALF, x, w, yi, z, b);
    DEBUG_MSG("[DEBUG] new downsample : "
              << identity.to(at::kFloat).sum().item<float>());
  } else {
    yi = x;
  }
  ...
  run_conv_scale_bias_add_activation(outdimA2, padA, convstrideA, dilationA,
                                     filterdimA3, outdimA3, CUDNN_DATA_HALF, y2,
                                     w, y3, z, b, yi);
...
std::vector<at::Tensor> bottleneck_backward(bool explicit_nhwc, int stride_1X1,
                                            std::vector<at::Tensor> inputs) {
  ...
  DEBUG_MSG(
      "[DEBUG] new dconv3 : " << inputs[10].to(at::kFloat).sum().item<float>());

  // wgrad
  auto wgrad3 = at::empty_like(inputs[3]);
  at::Half *dw3 = wgrad3.data_ptr<at::Half>();
  run_dconv(outdimA2, padA, convstrideA, dilationA, filterdimA3, outdimA3,
            CUDNN_DATA_HALF, conv_in, dw3, dy3,
            CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR);
  ...
  run_dconv_drelu_dscale(outdimA2, padA, convstrideA, dilationA, filterdimA3,
                         outdimA3, CUDNN_DATA_HALF, dy2, w, dy3, z, relu2);

  DEBUG_MSG(
      "[DEBUG] new dconv2 : " << grad_out2.to(at::kFloat).sum().item<float>());
  ...
  run_dconv(outdimA1, padA1, convstrideA, dilationA, filterdimA2, outdimA2,
            CUDNN_DATA_HALF, conv_in, dw2, dy2,
            CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR);
  ...
  // fused dgrad
  run_dconv_drelu_dscale(outdimA1, padA1, convstrideA, dilationA, filterdimA2,
                         outdimA2, CUDNN_DATA_HALF, dy1, w, dy2, z, relu1);
  ...
  */
  DEBUG_MSG(
      "[DEBUG] new dconv1 : " << grad_out1.to(at::kFloat).sum().item<float>());
  ...
  if (stride_1X1 != 1 || filterdimA3[0] != dimA[1]) {
    ...
    if (requires_grad) {
      run_dconv(dimA, padA, convstride1X1, dilationA, filterdimA4, outdimA3,
                CUDNN_DATA_HALF, dx_conv4, w, dy_conv4,
                CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_DATA_DESCRIPTOR);
      // we don't print here since we can't hook out this grad in pytorch alone
      // to compare, due to addition with dx DEBUG_MSG("[DEBUG] new dx_identity
      // : " << grad_x_conv4.to(at::kFloat).sum().item<float>());
    }
    // wgrad
    wgrad4 = at::empty_like(inputs[14]);
    at::Half *dw4 = wgrad4.data_ptr<at::Half>();
    run_dconv(dimA, padA, convstride1X1, dilationA, filterdimA4, outdimA3,
              CUDNN_DATA_HALF, x, dw4, dy_conv4,
              CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR);
  } else {
  ...
  run_dconv(dimA, padA, convstride1X1, dilationA, filterdimA1, outdimA1,
            CUDNN_DATA_HALF, x, dw1, dy1,
            CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR);
  ...
  if (requires_grad) {
    if (stride_1X1 != 1) {
      run_dconv(dimA, padA, convstride1X1, dilationA, filterdimA1, outdimA1,
                CUDNN_DATA_HALF, dx, w, dy1,
                CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_DATA_DESCRIPTOR);
      // add 2 together
      grad_x.add_(grad_x_conv4);
    } else {
      run_dconv_add(dimA, padA, convstride1X1, dilationA, filterdimA1, outdimA1,
                    CUDNN_DATA_HALF, dx, w, dy1, dx_conv4);
    }
  }

  DEBUG_MSG("[DEBUG] new dx : " << grad_x.to(at::kFloat).sum().item<float>());
  DEBUG_MSG(
      "[DEBUG] new wgrad1 : " << wgrad1.to(at::kFloat).sum().item<float>());
  DEBUG_MSG(
      "[DEBUG] new wgrad2 : " << wgrad2.to(at::kFloat).sum().item<float>());
  DEBUG_MSG(
      "[DEBUG] new wgrad3 : " << wgrad3.to(at::kFloat).sum().item<float>());
  ...
  if (stride_1X1 != 1 || filterdimA3[0] != dimA[1]) {
    DEBUG_MSG(
        "[DEBUG] new wgrad4 : " << wgrad4.to(at::kFloat).sum().item<float>());
...
masked_convbias_descriptors create_conv_bias_add_act_mask_descriptors(
    int64_t *x_dim_padded, int64_t *padA, int64_t *convstrideA,
    int64_t *dilationA, int64_t *w_dim_padded, int64_t *y_dim_padded,
  ...
  return masked_convbias_descriptors(
      cudnn_frontend::TensorBuilder()
          .setDim(4, x_dim_padded)
          .setStrides(4, x_stride_padded)
          .setId('x')
...
create_dconv_add_descriptors(int64_t *x_dim_padded, int64_t *padA,
                             int64_t *convstrideA, int64_t *dilationA,
                             int64_t *w_dim_padded, int64_t *y_dim_padded,
                             cudnnDataType_t dataType) {
  const int convDim = 2;
...
dconv_mask_descriptors create_dconv_mask_descriptors(
    int64_t *x_dim_padded, int64_t *padA, int64_t *convstrideA,
    int64_t *dilationA, int64_t *w_dim_padded, int64_t *y_dim_padded,
    int64_t *threshold_dim, cudnnDataType_t dataType) {
  const int convDim = 2;
...
void run_conv_add_scale_bias_activation(
    int64_t *x_dim_padded, int64_t *pad, int64_t *convstride, int64_t *dilation,
    int64_t *w_dim_padded, int64_t *y_dim_padded, cudnnDataType_t dataType,
    at::Half *devPtrX, at::Half *devPtrW, at::Half *devPtrY, at::Half *devPtrZ,
    at::Half *devPtrB, at::Half *devPtrI) {
  ...
  try {
    ...
    // activation
    std::array<cudnn_frontend::Operation const *, 5> ops = {
        &conv_op, &add_op, &scale_op, &bias_op, &act_op};
...
void run_conv_scale_bias_add_activation_mask(
    int64_t *x_dim_padded, int64_t *pad, int64_t *convstride, int64_t *dilation,
    int64_t *w_dim_padded, int64_t *y_dim_padded, int64_t *threshold_dim,
    cudnnDataType_t dataType, at::Half *devPtrX, at::Half *devPtrW,
    at::Half *devPtrY, at::Half *devPtrZ, at::Half *devPtrB, at::Half *devPtrI,
    int *devPtrT, int *devPtrU, int axis) {
  cudnnHandle_t handle_ = torch::native::getCudnnHandle();
  std::stringstream log_buf;
  try {
    int convDim = 2;
    ...
    // Creates the necessary tensor descriptors
    masked_convbias_descriptors tensors =
        create_conv_bias_add_act_mask_descriptors(
            x_dim_padded, pad, convstride, dilation, w_dim_padded, y_dim_padded,
            threshold_dim, dataType);
    DEBUG_CUDNN_MSG(log_buf, std::get<X_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf, std::get<Y_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf, std::get<W_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf, std::get<Z_TENSOR>(tensors).describe());
    ...
    DEBUG_CUDNN_MSG(log_buf,
                    std::get<THRESHOLD_TOP_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf,
                    std::get<THRESHOLD_BOTTOM_TENSOR>(tensors).describe());
    ...
    auto genIndex_op = cudnn_frontend::OperationBuilder(
                           CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
    ...
    auto lessThan_op = cudnn_frontend::OperationBuilder(
                           CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
    ...
    auto logicalOr_op = cudnn_frontend::OperationBuilder(
                            CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
    ...
    auto selection_op = cudnn_frontend::OperationBuilder(
                            CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR)
    ...
    if (devPtrI) {
      ...
      std::array<cudnn_frontend::Operation const *, 10> ops = {
          &conv_op,      &scale_op,    &bias_op,     &add_op,
          &act_op,       &genIndex_op, &lessThan_op, &greaterThan_op,
          &logicalOr_op, &selection_op};
      ...
      void *data_ptrs[] = {devPtrX, devPtrY, devPtrW, devPtrZ,
                           devPtrB, devPtrI, devPtrT, devPtrU};
      ...
      std::array<cudnn_frontend::Operation const *, 9> ops = {
          &conv_op,        &scale_op,     &bias_op,
...
void run_dconv_add_drelu_dscale(int64_t *x_dim_padded, int64_t *pad,
                                int64_t *convstride, int64_t *dilation,
                                int64_t *w_dim_padded, int64_t *y_dim_padded,
                                cudnnDataType_t dataType, at::Half *devPtrX,
                                at::Half *devPtrW, at::Half *devPtrY,
  ...
  try {
    ...
    dconv_add_descriptors tensors =
        create_dconv_add_descriptors(x_dim_padded, pad, convstride, dilation,
                                     w_dim_padded, y_dim_padded, dataType);
    DEBUG_CUDNN_MSG(log_buf, std::get<X_OR_DX_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf, std::get<DY_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf, std::get<W_OR_DW_TENSOR>(tensors).describe());
    ...
    DEBUG_CUDNN_MSG(log_buf,
                    std::get<DGRAD_OPTIONAL_TENSOR>(tensors).describe());
...
void run_dconv_drelu_dscale_mask(int64_t *x_dim_padded, int64_t *pad,
                                 int64_t *convstride, int64_t *dilation,
                                 int64_t *w_dim_padded, int64_t *y_dim_padded,
  ...
  try {
    ...
    DEBUG_CUDNN_MSG(log_buf,
                    std::get<DGRAD_OPTIONAL_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf,
                    std::get<DGRAD_GEN_INDEX_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf,
                    std::get<DGRAD_MASK_TOP_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf,
                    std::get<DGRAD_MASK_BOTTOM_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf, std::get<DGRAD_MASK_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(log_buf,
                    std::get<DGRAD_THRESHOLD_TOP_TENSOR>(tensors).describe());
    DEBUG_CUDNN_MSG(
        log_buf, std::get<DGRAD_THRESHOLD_BOTTOM_TENSOR>(tensors).describe());
    ...
    // Create an Operation Graph. In this case it is convolution add bias
    // activation
    std::array<cudnn_frontend::Operation const *, 8> ops = {
        &conv_op,     &act_op,         &scale_op,     &genIndex_op,
...
struct bottleneck_forward_status {
  ...
  void init(bool explicit_nhwc, int stride_1X1,
            std::vector<at::Tensor> inputs) {
    ...
    filterdimA2hh[0] = filterdimA2hh[1] = filterdimA2hh[2] = filterdimA2hh[3] =
        0;
    ...
    outdimA2[1] = filterdimA2[0];
    for (int dim = 0; dim < 2; dim++) {
      outdimA2[dim + 2] = getFwdConvOutputDim(outdimA1[dim + 2], padA1[dim],
                                              filterdimA2[dim + 2],
                                              convstrideA[dim], dilationA[dim]);
    }
    ...
    outdimA3[1] = filterdimA3[0];
    for (int dim = 0; dim < 2; dim++) {
      outdimA3[dim + 2] = getFwdConvOutputDim(outdimA2[dim + 2], padA[dim],
                                              filterdimA3[dim + 2],
                                              convstrideA[dim], dilationA[dim]);
    }
...
bottleneck_forward_init(bool explicit_nhwc, int stride_1X1,
                        std::vector<at::Tensor> inputs) {
...
// inputs contains x,w,z,b,(i)
void bottleneck_forward_out1(bool explicit_nhwc, int stride_1X1,
                             std::vector<at::Tensor> inputs,
                             std::vector<at::Tensor> outputs) {
  ...
  run_conv_scale_bias_add_activation(
      forward_state.dimA, forward_state.padA, forward_state.convstride1X1,
      forward_state.dilationA, forward_state.filterdimA1,
      forward_state.outdimA1, CUDNN_DATA_HALF, x, w, y1, z, b, nullptr);
...
at::Tensor bottleneck_forward_out2_halo(bool explicit_nhwc,
                                        at::Tensor fat_halo_y1,
                                        std::vector<at::Tensor> inputs) {
  ...
  auto halo_y2 =
      at::empty(forward_state.outdim4, inputs[0].type(), output_format);
  at::Half *y2 = halo_y2.data_ptr<at::Half>();

  run_conv_scale_bias_add_activation(
      forward_state.outdimA0, forward_state.padA2, forward_state.convstrideA,
      forward_state.dilationA, forward_state.filterdimA2,
      forward_state.outdimA4, CUDNN_DATA_HALF, y1, w, y2, z, b, nullptr);
...
at::Tensor bottleneck_forward_out2_halo_corr(bool explicit_nhwc,
                                             at::Tensor slim_halo_y1,
                                             std::vector<at::Tensor> inputs,
                                             at::Tensor w1by3,
  ...
  run_conv_add_scale_bias_activation(
      forward_state.outdimA4, forward_state.padA2, forward_state.convstrideA,
      forward_state.dilationA, forward_state.filterdimA2hh,
      forward_state.outdimA4, CUDNN_DATA_HALF, y1, w, y2, z, b, prev_out2);

  return halo_y2;
}

void bottleneck_forward_out2(bool explicit_nhwc, int stride_1X1,
                             std::vector<at::Tensor> inputs,
                             std::vector<at::Tensor> outputs) {
  ...
  run_conv_scale_bias_add_activation(
      forward_state.outdimA1, forward_state.padA1, forward_state.convstrideA,
      forward_state.dilationA, forward_state.filterdimA2,
      forward_state.outdimA2, CUDNN_DATA_HALF, y1, w, y2, z, b, nullptr);
  DEBUG_MSG("[DEBUG] new relu2 : " << out2.to(at::kFloat).sum().item<float>());
}

void bottleneck_forward_out2_mask(bool explicit_nhwc, int stride_1X1,
                                  std::vector<at::Tensor> inputs,
                                  std::vector<at::Tensor> outputs,
                                  at::Tensor thresholdTop,
  ...
  run_conv_scale_bias_add_activation_mask(
      forward_state.outdimA1, forward_state.padA1, forward_state.convstrideA,
      forward_state.dilationA, forward_state.filterdimA2,
      forward_state.outdimA2, forward_state.threshdim, CUDNN_DATA_HALF, y1, w,
      y2, z, b, nullptr, thresholdTop.data_ptr<int>(),
      thresholdBottom.data_ptr<int>(),
      2); // axis == 1 -> Does this assume explicit NHWC?
  DEBUG_MSG("[DEBUG] new relu2 : " << out2.to(at::kFloat).sum().item<float>());
}

void bottleneck_forward_out2_pad(bool explicit_nhwc, int stride_1X1,
                                 std::vector<at::Tensor> inputs,
                                 std::vector<at::Tensor> outputs,
                                 at::Tensor out1_pad) {
  ...
  // {%d,%d,%d,%d}\n",forward_state.outdimA2[0],forward_state.outdimA2[1],forward_state.outdimA2[2],forward_state.outdimA2[3]);
  run_conv_scale_bias_add_activation(
      forward_state.outdimA1b, forward_state.padA2, forward_state.convstrideA,
      forward_state.dilationA, forward_state.filterdimA2,
      forward_state.outdimA2, CUDNN_DATA_HALF, y1, w, y2, z, b, nullptr);
  DEBUG_MSG("[DEBUG] new relu2 : " << out2.to(at::kFloat).sum().item<float>());
}

void bottleneck_forward_rest(bool explicit_nhwc, int stride_1X1,
                             std::vector<at::Tensor> inputs,
                             std::vector<at::Tensor> outputs) {
  ...
  if (stride_1X1 != 1 ||
      forward_state.filterdimA3[0] != forward_state.dimA[1]) {

    w = inputs[10].data_ptr<at::Half>();
    z = inputs[11].data_ptr<at::Half>();
    b = inputs[12].data_ptr<at::Half>();
    run_conv_scale_bias(forward_state.dimA, forward_state.padA,
                        forward_state.convstride1X1, forward_state.dilationA,
                        forward_state.filterdimA4, forward_state.outdimA3,
                        CUDNN_DATA_HALF, x, w, yi, z, b);
    DEBUG_MSG("[DEBUG] new downsample : "
              << identity.to(at::kFloat).sum().item<float>());
  } else {
    yi = x;
  ...
  run_conv_scale_bias_add_activation(
      forward_state.outdimA2, forward_state.padA, forward_state.convstrideA,
      forward_state.dilationA, forward_state.filterdimA3,
      forward_state.outdimA3, CUDNN_DATA_HALF, y2, w, y3, z, b, yi);
  DEBUG_MSG("[DEBUG] new relu3 : " << out3.to(at::kFloat).sum().item<float>());
}
...
std::vector<at::Tensor>
bottleneck_backward_init(bool explicit_nhwc, int stride_1X1,
                         std::vector<at::Tensor> inputs) {
  ...
  outputs.push_back(wgrad3);
  if (stride_1X1 != 1 ||
      backward_state.filterdimA3[0] != backward_state.dimA[1]) {
...
void bottleneck_backward_wgrad3(bool explicit_nhwc, int stride_1X1,
                                std::vector<at::Tensor> inputs,
                                std::vector<at::Tensor> outputs) {
  ...
  at::Half *dw3 = wgrad3.data_ptr<at::Half>();
  run_dconv(backward_state.outdimA2, backward_state.padA,
            backward_state.convstrideA, backward_state.dilationA,
            backward_state.filterdimA3, backward_state.outdimA3,
            CUDNN_DATA_HALF, conv_in, dw3, dy3,
            CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR);
  DEBUG_MSG(
      "[DEBUG] new wgrad3 : " << wgrad3.to(at::kFloat).sum().item<float>());
...
at::Tensor bottleneck_backward_grad_out2(bool explicit_nhwc, int stride_1X1,
                                         std::vector<at::Tensor> inputs,
                                         std::vector<at::Tensor> outputs) {
  ...
  // dgrad
  auto grad_out2 =
      at::empty(backward_state.outdim2, inputs[0].type(), output_format);
  at::Half *dy2 = grad_out2.data_ptr<at::Half>();
  at::Half *w = inputs[3].data_ptr<at::Half>();
  at::Half *z = inputs[5].data_ptr<at::Half>();
  ...
  run_dconv_drelu_dscale(backward_state.outdimA2, backward_state.padA,
                         backward_state.convstrideA, backward_state.dilationA,
                         backward_state.filterdimA3, backward_state.outdimA3,
                         CUDNN_DATA_HALF, dy2, w, dy3, z, relu2);
...
at::Tensor bottleneck_backward_grad_out1(bool explicit_nhwc, int stride_1X1,
                                         std::vector<at::Tensor> inputs,
                                         std::vector<at::Tensor> outputs,
                                         at::Tensor grad_out2) {
  ...
  // dgrad
  auto grad_out1 =
      at::empty(backward_state.outdim1, inputs[0].type(), output_format);
  at::Half *dy1 = grad_out1.data_ptr<at::Half>();
  at::Half *w = inputs[2].data_ptr<at::Half>();
  at::Half *z = inputs[4].data_ptr<at::Half>();
  ...
  run_dconv_drelu_dscale(backward_state.outdimA1, backward_state.padA1,
                         backward_state.convstrideA, backward_state.dilationA,
                         backward_state.filterdimA2, backward_state.outdimA2,
                         CUDNN_DATA_HALF, dy1, w, dy2, z, relu1);

  return grad_out1;
}

at::Tensor bottleneck_backward_grad_out1_mask(
    bool explicit_nhwc, int stride_1X1, std::vector<at::Tensor> inputs,
  ...
  run_dconv_drelu_dscale_mask(
      backward_state.outdimA1, backward_state.padA1, backward_state.convstrideA,
      backward_state.dilationA, backward_state.filterdimA2,
      backward_state.outdimA2, backward_state.threshdim, CUDNN_DATA_HALF, dy1,
      w, dy2, z, relu1, thresholdTop.data_ptr<int>(),
      thresholdBottom.data_ptr<int>(), 2);

  return grad_out1;
}

// perform backward data 1x3 convolution (grad_out * w_rot180) on grad_out2
// input of shape [N,1,W,C] with padding=(0,1) to produce output of shape
// [N,1,W,C]
at::Tensor bottleneck_backward_grad_out1_halo_corr(
    bool explicit_nhwc, int stride_1X1, std::vector<at::Tensor> inputs,
  ...
  // dgrad
  auto grad_out1_halo =
      at::empty(backward_state.outdim1hh, inputs[0].type(), output_format);
  at::Half *dy1h = grad_out1_halo.data_ptr<at::Half>();
  // at::Half* w = inputs[2].data_ptr<at::Half>();  // use w1by3 instead, which
  // is a sliced version of inputs[2]
  ...
  run_dconv_add_drelu_dscale(backward_state.outdimA1hh,
                             ...
                             backward_state.outdimA2hh, CUDNN_DATA_HALF, dy1h,
                             w, dy2h, z, relu1h, pdy1h);

  return grad_out1_halo;
}

// perform backward data 3x3 convolution (grad_out * w_rot180) on grad_out2
// input of shape [N,3,W,C] with padding=(1,1) to produce output of shape
// [N,3,W,C]
at::Tensor bottleneck_backward_grad_out1_halo(bool explicit_nhwc,
                                              int stride_1X1,
                                              std::vector<at::Tensor> inputs,
                                              std::vector<at::Tensor> outputs,
  ...
  // dgrad
  auto grad_out1_halo =
      at::empty(backward_state.outdim1h, inputs[0].type(), output_format);
  at::Half *dy1h = grad_out1_halo.data_ptr<at::Half>();
  at::Half *w = inputs[2].data_ptr<at::Half>();
  at::Half *z = inputs[4].data_ptr<at::Half>();
  ...
  // printf("backward_state.filterdimA2 =
  // {%d,%d,%d,%d}\n",backward_state.filterdimA2[0],backward_state.filterdimA2[1],backward_state.filterdimA2[2],backward_state.filterdimA2[3]);
  run_dconv_drelu_dscale(backward_state.outdimA1h, backward_state.padA1,
                         backward_state.convstrideA, backward_state.dilationA,
                         backward_state.filterdimA2, backward_state.outdimA2h,
                         CUDNN_DATA_HALF, dy1h, w, dy2h, z, relu1h);

  return grad_out1_halo;
}

void bottleneck_backward_wgrad2_pad(bool explicit_nhwc, int stride_1X1,
                                    std::vector<at::Tensor> inputs,
                                    std::vector<at::Tensor> outputs,
...
void bottleneck_backward_wgrad2(bool explicit_nhwc, int stride_1X1,
                                std::vector<at::Tensor> inputs,
                                std::vector<at::Tensor> outputs,
                                at::Tensor grad_out2) {
  ...
  run_dconv(backward_state.outdimA1, backward_state.padA1,
            backward_state.convstrideA, backward_state.dilationA,
            backward_state.filterdimA2, backward_state.outdimA2,
            CUDNN_DATA_HALF, conv_in, dw2, dy2,
            CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR);
...
// compute halo cells for input volume of dimension [N,1,W,C] with padding=(0,1)
// to produce output volume of dimension [N,1,W,C] input and grad_out2_halo
// tensors are all of same shape output tensor is of shape [Cin,1,3,Cout]
// (regular filter dims are [Cin,3,3,Cout]
at::Tensor bottleneck_backward_wgrad2_halo(bool explicit_nhwc, int stride_1X1,
                                           std::vector<at::Tensor> inputs,
                                           std::vector<at::Tensor> outputs,
                                           at::Tensor input,
                                           at::Tensor grad_out2_halo) {
  ...
  // wgrad
  auto wgrad2_halo =
      at::empty(backward_state.filterdim2hh, input.type(), output_format);
...
void bottleneck_backward_wgrad1(bool explicit_nhwc, int stride_1X1,
                                std::vector<at::Tensor> inputs,
                                std::vector<at::Tensor> outputs,
                                at::Tensor grad_out1) {
  ...
  at::Half *dw1 = wgrad1.data_ptr<at::Half>();
  run_dconv(backward_state.dimA, backward_state.padA,
            backward_state.convstride1X1, backward_state.dilationA,
            backward_state.filterdimA1, backward_state.outdimA1,
            CUDNN_DATA_HALF, x, dw1, dy1,
            CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR);
}

void bottleneck_backward_rest(bool explicit_nhwc, int stride_1X1,
                              std::vector<at::Tensor> inputs,
                              std::vector<at::Tensor> outputs,
                              at::Tensor grad_out2, at::Tensor grad_out1) {
  ...
  */
  ...
  if (stride_1X1 != 1 ||
    ...
    if (requires_grad) {
      run_dconv(backward_state.dimA, backward_state.padA,
                backward_state.convstride1X1, backward_state.dilationA,
                backward_state.filterdimA4, backward_state.outdimA3,
                CUDNN_DATA_HALF, dx_conv4, w, dy_conv4,
                CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_DATA_DESCRIPTOR);
      // we don't print here since we can't hook out this grad in pytorch alone
      // to compare, due to addition with dx DEBUG_MSG("[DEBUG] new dx_identity
  ...
  if (requires_grad) {
    if (stride_1X1 != 1) {
      ...
      run_dconv_add(backward_state.dimA, backward_state.padA,
                    backward_state.convstride1X1, backward_state.dilationA,
                    backward_state.filterdimA1, backward_state.outdimA1,
                    CUDNN_DATA_HALF, dx, w, dy1, dx_conv4);
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("forward_out2", &bottleneck_forward_out2, "Bottleneck block forward",
        py::call_guard<py::gil_scoped_release>());
  m.def("forward_out2_mask", &bottleneck_forward_out2_mask,
        "Bottleneck block forward", py::call_guard<py::gil_scoped_release>());
  m.def("forward_out2_halo", &bottleneck_forward_out2_halo,
        "Bottleneck block forward", py::call_guard<py::gil_scoped_release>());
  m.def("forward_out2_halo_corr", &bottleneck_forward_out2_halo_corr,
        "Bottleneck block forward", py::call_guard<py::gil_scoped_release>());
  m.def("forward_out2_pad", &bottleneck_forward_out2_pad,
        "Bottleneck block forward", py::call_guard<py::gil_scoped_release>());
  m.def("forward_rest", &bottleneck_forward_rest, "Bottleneck block forward",
        py::call_guard<py::gil_scoped_release>());
  m.def("backward_init", &bottleneck_backward_init,
        "Bottleneck block backward init",
        py::call_guard<py::gil_scoped_release>());
  m.def("backward_grad_out2", &bottleneck_backward_grad_out2,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
  m.def("backward_grad_out1", &bottleneck_backward_grad_out1,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
  m.def("backward_grad_out1_mask", &bottleneck_backward_grad_out1_mask,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
  m.def("backward_grad_out1_halo", &bottleneck_backward_grad_out1_halo,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
  m.def("backward_grad_out1_halo_corr",
        &bottleneck_backward_grad_out1_halo_corr, "Bottleneck block backward",
        py::call_guard<py::gil_scoped_release>());
  m.def("backward_wgrad2_pad", &bottleneck_backward_wgrad2_pad,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
  m.def("backward_wgrad2", &bottleneck_backward_wgrad2,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
  m.def("backward_wgrad2_halo", &bottleneck_backward_wgrad2_halo,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
  m.def("backward_wgrad3", &bottleneck_backward_wgrad3,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
  m.def("backward_wgrad1", &bottleneck_backward_wgrad1,
        "Bottleneck block backward", py::call_guard<py::gil_scoped_release>());
