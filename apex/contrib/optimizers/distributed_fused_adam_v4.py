...
class DistributedFusedAdamV4(torch.optim.Optimizer):
    ...
    def __init__(self, params,
        ...
        super(DistributedFusedAdamV4, self).__init__(params, defaults)
        ...
        self._new_params_blocks, self._new_params_chunks, self._new_params_shards = _flat_split(self._new_params)
        ...
        if not self._do_not_flatten_model:
            with torch.no_grad():
                for p, grads_info in zip(self._model_params, self._grads_info):
                    ...
                    p.set_(new_param_view.view_as(p))
        ...
        with torch.cuda.stream(ag_stream):
            ...
            if not self._do_not_flatten_model:
                for chunk_id in range(self._num_chunks):
                    ...
                    glob_chunk_id = block_id * self._num_chunks + chunk_id
                    torch.distributed.all_gather(self._new_params_shards[block_id][chunk_id], self._fp16_p[block_id][chunk_id], group=self._ag_pg[glob_chunk_id%self._num_ag_pg], no_copy=True)
        if block_id == 0:
    ...
    def _pipeline_step(self):
        if not self._do_not_flatten_model:
            ...
            for block_id in range(self._num_blocks):
                for chunk_id in range(self._num_chunks):
                    glob_chunk_id = block_id * self._num_chunks + chunk_id
                    ag_stream = self._ag_st[glob_chunk_id%self._num_ag_pg]
                    with torch.cuda.stream(ag_stream):
                        self._reductions_works[block_id][chunk_id].wait()
                        self.__launch_kernel(
                                self._fp32_p_chunks[block_id][chunk_id],
                                self._fp16_p_chunks[block_id][chunk_id],
                                self._fp32_m_chunks[block_id][chunk_id],
                                self._fp32_v_chunks[block_id][chunk_id],
                                self._fp16_g_chunks[block_id][chunk_id])
                        torch.distributed.all_gather(self._new_params_shards[block_id][chunk_id], self._fp16_p[block_id][chunk_id], group=self._ag_pg[glob_chunk%self._num_ag_pg], no_copy=True)
                    self._completion_st.wait_stream(ag_stream)
        else:
