...
import fast_cublaslt_encdec_multihead_attn_norm_add

class EncdecAttnNormAddFunc(torch.autograd.Function) :
    @staticmethod
...
encdec_attn_norm_add_func = EncdecAttnNormAddFunc.apply

class EncdecAttnNormAddCublasltFunc(torch.autograd.Function) :
    @staticmethod
    def forward(ctx, use_time_mask, is_training, heads, inputs_q, inputs_kv, lyr_nrm_gamma_weights, lyr_nrm_beta_weights, input_weights_q, input_weights_kv, output_weights, pad_mask, dropout_prob) :
        heads_t        = Variable(torch.tensor([heads]))
        dropout_prob_t = Variable(torch.tensor([dropout_prob]))
        ...
        dropout_add_mask,                                               \
        outputs =                                                       \
            fast_cublaslt_encdec_multihead_attn_norm_add.forward(       \
                              use_mask,                                 \
                              use_time_mask,                            \
                              is_training,                              \
                              heads,                                    \
    ...
    def backward(ctx, output_grads) :
        ...
        input_weight_kv_grads,                                          \
        output_weight_grads    =                                        \
            fast_cublaslt_encdec_multihead_attn_norm_add.backward(      \
                              heads_t[0],                               \
                              output_grads,                             \
                              matmul2_results,                          \
                              dropout_results,                          \
...
encdec_attn_norm_add_cublaslt_func = EncdecAttnNormAddCublasltFunc.apply

class EncdecMultiheadAttnNormAdd(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., softmax_type='default', cublaslt=False, bias=False):
        ...
        if self.cublaslt :
            self.attn_func = encdec_attn_norm_add_cublaslt_func
            fast_cublaslt_encdec_multihead_attn_norm_add.forward_init(self.num_heads, self.embed_dim)
            fast_cublaslt_encdec_multihead_attn_norm_add.backward_init(self.num_heads, self.embed_dim)
        else :
            self.attn_func = encdec_attn_norm_add_func
    
    def __del__(self) :
        if self.cublaslt :
            fast_cublaslt_encdec_multihead_attn_norm_add.forward_del()
            fast_cublaslt_encdec_multihead_attn_norm_add.backward_del()
    ...
    def forward(self, query, key, value, is_training=True, mask_future_timesteps=False,
        ...
        outputs = self.attn_func(False, is_training, self.num_heads, query, key, self.lyr_nrm_gamma_weights, self.lyr_nrm_beta_weights, self.in_proj_weight_q, self.in_proj_weight_kv, self.out_proj_weight, key_padding_mask, self.dropout)
        return outputs,None
