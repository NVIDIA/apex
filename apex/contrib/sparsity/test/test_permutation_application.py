    ...
    def __init__(
        ...
        for c in range(self.num_convs - 1):
            self.conv_stack.add_module(
                f"conv_{c}",
                torch.nn.Conv2d(
                    self.channels, self.channels, kernel_size=(3, 3), padding=1
                ),
            )
            self.expected_C_params += 1
            ...
            if self.normalization == "BatchNorm2d":
                ...
                self.expected_C_params -= (
                    1  # GN prevents permutations of the neighboring convs
                )
                self.expected_K_params -= 2
                ...
                self.conv_stack.add_module(
                    ...
                    torch.nn.InstanceNorm2d(
                        self.channels, affine=True, track_running_stats=False
                    ),
                )
                self.expected_K_params += 2
                ...
                self.conv_stack.add_module(
                    f"norm_{c}", torch.nn.LayerNorm([self.channels, 7, 7])
                )
                self.expected_K_params += 2
...
class conv_1d(torch.nn.Module):
    ...
    def __init__(
        ...
        self.input_conv = torch.nn.Conv2d(
            self.input_shape[1], 32, kernel_size=(3, 3), padding=1
        )
        self.expected_K_params += 2
    ...
    def forward(self, x: torch.Tensor):
        ...
        s0shape = step0.shape
        step1 = self.branch_a_1D(
            step0.view(s0shape[0], s0shape[1], s0shape[2] * s0shape[3])
        ).view(s0shape)
        if self.with_2d:
...
class grouped_convs(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv_stack.add_module(
            "conv_0",
            torch.nn.Conv2d(
                self.channels, self.channels, kernel_size=(3, 3), padding=1, groups=1
            ),
        ...
        self.conv_stack.add_module(
            "conv_gr2",
            torch.nn.Conv2d(
                self.channels, self.channels, kernel_size=(3, 3), padding=1, groups=2
            ),
        )
...
class simple_forks_joins(torch.nn.Module):
    ...
    def __init__(
        ...
        self.block_a.add_module(
            "conv_a0",
            torch.nn.Conv2d(
                self.channels, self.channels // 2, kernel_size=(3, 3), padding=1
            ),
        )
        self.block_a.add_module(
            "conv_a1",
            torch.nn.Conv2d(
                self.channels // 2, self.channels, kernel_size=(3, 3), padding=1
            ),
        )
...
class coparent_poison(torch.nn.Module):
    ...
    def __init__(
        ...
        self.block_b = torch.nn.Sequential()
        self.block_b.add_module(
            "conv_b0", torch.nn.Conv2d(128, 128, kernel_size=(3, 3), padding=1)
        )
        self.block_b.add_module(
            "conv_b1",
...
class depthwise_child_is_sibling(torch.nn.Module):
    ...
    def __init__(
        ...
        self.expected_C_params += 1
        self.output_conv.add_module(
            "output_conv", torch.nn.Conv2d(128, 8, kernel_size=(1, 1))
        )
...
class module_attribute(torch.nn.Module):
    ...
    def __init__(
        ...
        self.expected_C_params += 1
        self.output_conv.add_module(
            "conv_output", torch.nn.Conv2d(128, 8, kernel_size=(3, 3))
        )
...
class square_attribute(torch.nn.Module):
    ...
    def __init__(
        ...
        self.input_linear.add_module(
            "linear_input", torch.nn.Linear(self.input_shape[1], 16)
        )
...
class one_sparse_sibling(torch.nn.Module):
    ...
    def __init__(
        ...
        self.block_a.add_module(
            "conv_a1", torch.nn.Conv2d(3, 128, kernel_size=(3, 3), padding=1)
        )
        ...
        self.block_b.add_module(
            "conv_b1", torch.nn.Conv2d(128, 128, kernel_size=(1, 1))
        )

        self.out_conv = torch.nn.Sequential()
        self.expected_C_params += 1
        self.out_conv.add_module(
            "conv_out", torch.nn.Conv2d(128, 8, kernel_size=(1, 1))
        )
...
class test_concat(torch.nn.Module):
    ...
    def __init__(
        ...
        self.left_paths = torch.nn.ModuleList(
        ...
        for d in range(1, depth, 1):
            ...
            if dim == 1:
                out_C += 64
            self.left_paths.append(
                torch.nn.Conv2d(in_C + 64, out_C, kernel_size=(1, 1))
            )
            if dim == 1:
                in_C += 64
        ...
        self.expected_C_params += 1
        self.expected_K_params += 2
        self.right_path.add_module(
            "conv_b", torch.nn.Conv2d(64, 64 * ratio, kernel_size=(1, 1))
        )
        ...
        if dim == 1:
            out_C += 64 * ratio
        self.out_conv.add_module(
            "conv_out", torch.nn.Conv2d(out_C, 16, kernel_size=(1, 1))
        )
...
class test_trace_failure(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        channel_offset = (
            channel_offset.unsqueeze(0).unsqueeze(2).unsqueeze(3).expand_as(step0)
        )
...
def test_model(model, tag, verbosity=0, save_onnx=False):
        ...
        for p_name, p in module.named_parameters():
            ...
            if isinstance(
                ...
                if (
                    ...
                    sparse_parameters.append(
                        (module_name, module, p_name, p, mask, None)
                    )
    ...
    if verbosity > 1:
        ...
        all_param_names = [
            module_name + ":" + p_name
            for (module_name, module, p_name, p) in all_parameters
        ]
        print(
    ...
    Permutation.set_permutation_params_from_asp(
        model, sparse_parameters, all_parameters, verbosity
    )
...
def main():
    ...
    global_success &= test_model(
        module_attribute(complexity=1), "single attribute thrice"
    )
    global_success &= test_model(
        MHA_test(hidden_dim=256, seq_len=64, num_heads=16), "stacked MHA"
    )
    ...
    global_success &= test_model(
        test_flatten_op(change_dims=False), "useless flatten op"
    )
    global_success &= test_model(test_flatten_module(), "flatten module")
    global_success &= test_model(
        test_flatten_module(change_dims=False), "useless flatten module"
    )
