...
// find the magnitude after enforcing the 2:4 sparsity constraint on a group of
// 4 values
 ...
 **********************************************************/
__global__ void permute_and_sum_after_2_to_4(
    float *matrix, unsigned int rows, unsigned int cols, unsigned int *stripes,
...
void free_permutation_memory(float **dmatrix, unsigned int **dstripe_groups,
                             unsigned int **dpermutations, float **dresults,
                             float **hresults) {
  cudaFree(*dmatrix);
  cudaFree(*dresults);
...
int set_up_check_permutation_memory(
    float **dmatrix, unsigned int rows, unsigned int cols,
    unsigned int **dstripe_groups, unsigned int group_width,
    unsigned int num_groups, unsigned int **dpermutations,
    unsigned int num_permutations, float **dresults, float **hresults) {
  ...
  if (!allocated || setupRows != rows || setupCols != cols ||
      setupGroupWidth != group_width || setupNumGroups != num_groups ||
      setupNumPermutations != num_permutations) {

    if (allocated) {
      free_permutation_memory(dmatrix, dstripe_groups, dpermutations, dresults,
                              hresults);
    }

    gpuErrchk(cudaMalloc((void **)dmatrix, rows * cols * sizeof(float)));
    gpuErrchk(cudaMalloc((void **)dstripe_groups,
                         group_width * num_groups * sizeof(unsigned int)));
    gpuErrchk(
        cudaMalloc((void **)dpermutations,
                   num_permutations * group_width * 4 * sizeof(unsigned int)));
...
int run_check_permutations(
    ...
    py::array_t<unsigned int>
        &py_stripe_groups, // groups of stripes, group_width = stripes per
                           // group, num_groups = groups in the array
    unsigned int group_width, unsigned int num_groups,
    py::array_t<unsigned int>
        &py_permutations, // array of permutations to try, group_width*4 values
                          // per each of num_permutations permutations
    unsigned int num_permutations,
    py::array_t<float>
        &py_improvement, // improvment offered by the best permutation
  ...
  int fresh_alloc = set_up_check_permutation_memory(
      &d_matrix, rows, cols, &d_stripes, group_width, num_groups,
      &d_permutations, num_permutations, &d_results, &results);
  if (fresh_alloc == 1) {
    gpuErrchk(
        cudaMemcpy(d_permutations, permutations,
                   num_permutations * group_width * 4 * sizeof(unsigned int),
                   cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_stripes, stripe_groups,
                         group_width * num_groups * sizeof(unsigned int),
                         cudaMemcpyHostToDevice));
  }
  ...
  gpuErrchk(cudaMemcpy(d_matrix, matrix, rows * cols * sizeof(float),
                       cudaMemcpyHostToDevice));

  // get results for all permutations
  permute_and_sum_after_2_to_4<<<num_permutations, threads,
                                 threads * group_width * 4 * 2 *
                                     sizeof(float)>>>(
      d_matrix, rows, cols, d_stripes, group_width, d_permutations, d_results);
  gpuErrchk(cudaDeviceSynchronize());

  gpuErrchk(cudaMemcpy(results, d_results, num_permutations * sizeof(float),
 ...
 **********************************************************/
// find the magnitude after enforcing the 2:4 sparsity constraint on a subset of
// the columns of an input matrix
__global__ void subset_sum_after_2_to_4(float *matrix, unsigned int rows,
                                        unsigned int cols,
...
// each threads in the block handle all handle the same permutation at the same
// time on different rows before moving to the next permutation
__global__ void build_permute_map(float *matrix, unsigned int rows,
                                  unsigned int cols, unsigned int *stripes,
                                  unsigned int group_width,
                                  unsigned int *permutations,
                                  unsigned int num_permutations,
                                  unsigned int perm_length, float *output,
  ...
  unsigned int *stripe_group =
      (unsigned int *)&stripes[blockIdx.x * group_width];
  ...
  // each thread handles all permutations in the row before moving on to the
  // next row
  size_t num_rows = max((int)ceilf((float)rows / (float)blockDim.x), 1);
  size_t row_offset = num_rows * threadIdx.x;
...
#pragma unroll 4
  ...
  // at this point, each permutation's sum in this stripe group has been
  // calculated now, find the best option
  __syncthreads();
...
int set_up_sum_after_2_to_4_memory(float **dmatrix, unsigned int rows,
                                   unsigned int cols, float **dresult) {
  static unsigned int setupRows = 0;
  static unsigned int setupCols = 0;
  static bool allocated = false;
...
int run_subset_sum_after_2_to_4(py::array_t<float> &py_matrix,
  ...
  int fresh_allocation =
      set_up_sum_after_2_to_4_memory(&d_matrix, rows, cols, &d_result);
  ...
  subset_sum_after_2_to_4<<<blocks, threads>>>(d_matrix, rows, cols, start_col,
                                               end_col, d_result);
  gpuErrchk(cudaDeviceSynchronize());
...
void set_up_permute_map_memory(
    float **dmatrix, unsigned int rows, unsigned int cols,
    unsigned int **dstripes, unsigned int num_groups, unsigned int group_width,
    unsigned int **dpermutations, unsigned int num_permutations,
    unsigned int perm_length, float **doutput, unsigned int **dindices,
    float **hresult, unsigned int **hindices) {
  static unsigned int setUpRows = 0;
  static unsigned int setUpCols = 0;
  static unsigned int setUpGroupWidth = 0;
  static unsigned int setUpNumGroups = 0;
  ...
  if (setUpGroupWidth < group_width || setUpNumGroups < num_groups) {
    ...
    gpuErrchk(cudaMalloc((void **)dstripes,
                         num_groups * group_width * sizeof(unsigned int)));
    ...
    if (setUpNumGroups < num_groups) {
      ...
      gpuErrchk(
          cudaMalloc((void **)dindices, num_groups * sizeof(unsigned int)));
      if (*hindices != NULL) {
        free(*hindices);
        *hindices = NULL;
      }
  ...
  if (setUpNumPerms < num_permutations || setUpPermLength < perm_length) {
    ...
    gpuErrchk(
        cudaMalloc((void **)dpermutations,
                   perm_length * num_permutations * sizeof(unsigned int)));
  }
  ...
  if (setUpNumPerms < num_permutations || setUpNumGroups < num_groups) {
    ...
    gpuErrchk(cudaMalloc((void **)doutput,
                         num_permutations * num_groups * sizeof(float)));
    if (*hresult != NULL) {
      free(*hresult);
      *hresult = NULL;
    }
...
int run_build_permute_map(py::array_t<float> &py_matrix, unsigned int rows,
                          unsigned int cols,
                          py::array_t<unsigned int> &py_stripes,
                          unsigned int num_groups, unsigned int group_width,
  ...
  const unsigned int MAX_GROUPS_PER_LAUNCH =
      num_permutations <= 5775 ? 1820 : 40;
  ...
  set_up_permute_map_memory(&d_matrix, rows, cols, &d_stripes,
                            min(num_groups, MAX_GROUPS_PER_LAUNCH), group_width,
                            &d_permutations, num_permutations, perm_length,
                            &d_output, &d_indices, &hresult, &hindices);
  ...
  gpuErrchk(cudaMemcpy(d_matrix, matrix, rows * cols * sizeof(float),
                       cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpy(d_permutations, permutations,
                       num_permutations * perm_length * sizeof(unsigned int),
                       cudaMemcpyHostToDevice));

  unsigned int group_offset = 0;
  for (unsigned int l = 0; l < launches; ++l) {
    unsigned int groups_this_launch =
        (l < full_launches) ? MAX_GROUPS_PER_LAUNCH : final_launch;

    gpuErrchk(
        cudaMemcpy(d_stripes, &stripes[group_offset * group_width],
                   groups_this_launch * group_width * sizeof(unsigned int),
                   cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemset(
        d_output, 0, groups_this_launch * num_permutations * sizeof(float)));
    gpuErrchk(
        cudaMemset(d_indices, 0, groups_this_launch * sizeof(unsigned int)));

    unsigned int shmem = 32 * (32) * sizeof(float);
    build_permute_map<<<groups_this_launch, 32, shmem>>>(
        d_matrix, rows, cols, d_stripes, group_width, d_permutations,
        num_permutations, perm_length, d_output, d_indices);
    gpuErrchk(cudaDeviceSynchronize());

    gpuErrchk(cudaMemcpy(hresult, d_output,
                         num_permutations * groups_this_launch * sizeof(float),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(hindices, d_indices,
                         groups_this_launch * sizeof(unsigned int),
                         cudaMemcpyDeviceToHost));
 ...
 **********************************************************/
// find the magnitude improvement if some columns were swapped (check all pairs
// of columns in all the stripe_pairs)
__global__ void swap_columns_sum_after_2_to_4(float *matrix, unsigned int rows,
                                              unsigned int cols,
  ...
  // space for 32 threads, 8 values (2 stripes) in use at a time, plus 16
  // partial sums and one base sum
  ...
  *base_psum = 0.0f;
    ...
    *base_psum += group_2_to_4(*local_stripe0) + group_2_to_4(*local_stripe1);
    ...
    for (unsigned int c0 = 0; c0 < 4; ++c0) {
      for (unsigned int c1 = 0; c1 < 4; ++c1) {
        ...
        // grab the sum
        local_psum[swap_idx] +=
            group_2_to_4(*local_stripe0) + group_2_to_4(*local_stripe1);
  ...
  if (threadIdx.x == 0) {
    ...
    for (unsigned int swap = 0; swap < 16; ++swap) {
      atomicAdd(&output[blockIdx.x * 16 + swap],
                local_psum[swap] - (*base_psum));
    }
  }
}

void set_up_swap_map_memory(float **dmatrix, unsigned int rows,
                            unsigned int cols, unsigned int **dstripe_pairs,
  ...
  if (*dstripe_pairs == NULL || *dresult == NULL || setupPairs < num_pairs) {
    ...
    gpuErrchk(cudaMalloc((void **)dstripe_pairs,
                         num_pairs * 2 * sizeof(unsigned int)));
...
int run_build_swap_map(py::array_t<float> &py_matrix, unsigned int rows,
                       unsigned int cols,
  ...
  unsigned int *stripe_pairs = uint_ptr_from_numpy(
      py_stripe_pairs); //(unsigned int*)py_stripe_pairs.data();
  float *output = float_ptr_from_numpy(py_output); //(float*)py_output.data();

  unsigned int num_pairs = py_stripe_pairs.size() / 2;

  set_up_swap_map_memory(&d_matrix, rows, cols, &d_stripe_pairs, num_pairs,
                         &d_result);
  gpuErrchk(cudaMemcpy(d_matrix, matrix, rows * cols * sizeof(float),
                       cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpy(d_stripe_pairs, stripe_pairs,
                       2 * num_pairs * sizeof(unsigned int),
                       cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemset(d_result, 0, num_pairs * 16 * sizeof(float)));

  unsigned int shmem = 32 * (32) * sizeof(float);
  swap_columns_sum_after_2_to_4<<<num_pairs, 32, shmem>>>(
      d_matrix, rows, cols, d_stripe_pairs, d_result);
  gpuErrchk(cudaDeviceSynchronize());

  gpuErrchk(cudaMemcpy(output, d_result, num_pairs * 16 * sizeof(float),
                       cudaMemcpyDeviceToHost));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("sum_after_2_to_4", &run_subset_sum_after_2_to_4,
        "matrix sum after applying 2:4 (CUDA)",
        py::call_guard<py::gil_scoped_release>());
  m.def("build_permute_map", &run_build_permute_map,
        "optimize stripe groups (CUDA)",
        py::call_guard<py::gil_scoped_release>());
  m.def("check_permutations", &run_check_permutations,
        "exhaustively check all permutations (CUDA)",
        py::call_guard<py::gil_scoped_release>());
