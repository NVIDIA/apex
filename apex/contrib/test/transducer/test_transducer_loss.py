...
class TransducerLossTest(unittest.TestCase):
    ...
    def gen_input(self, scalar_t):
        ...
        self.x_len = torch.randint(T_min, T_max+1, (self.B,), dtype=torch.int, device=device) 
        self.y_len = torch.randint(U_min-1, U_max, (self.B,), dtype=torch.int, device=device)
        self.x_len[torch.randint(0, self.B, (1,)).item()] = T_max
    ...
    def _pack(self, x):
        ...
        for b in range(self.B):
            list_x_row = [x[b, t, : self.y_len[b]+1] for t in range(self.x_len[b])]
            x_row = torch.cat(list_x_row)
            list_x.append(x_row)
        x_packed = torch.cat(list_x).data.clone()
        x_packed.requires_grad = True
        batch_offset = torch.cumsum(self.x_len * (self.y_len+1), dim=0)
        return x_packed, batch_offset

    def _unpack(self, x):
        x_unpacked = torch.zeros(self.B, self.x_len.max(), self.y_len.max()+1, x.size(-1), 
                                    dtype=x.dtype, device=x.device)
        for b in range(self.B):
            my_batch_offset = 0 if b == 0 else self.batch_offset[b-1]
            my_f_len = self.x_len[b]
