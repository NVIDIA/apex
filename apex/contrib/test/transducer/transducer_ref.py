...
def transducer_loss_reference(x, label, x_len, y_len, blank_idx, loss_grad):
    def log_sum_exp(a, b):
        if (a >= b):
    ...
    def forward_alpha(x, label, x_len, y_len, blank_idx):
        B, T, U, V = x.size()
    ...
    def forward_beta(x, label, x_len, y_len, blank_idx):
        B, T, U, V = x.shape
        acc_t = torch.float32 if x.dtype in [torch.float16, torch.float32] else x.dtype
        beta = torch.zeros((B, T, U), dtype=acc_t, device=x.device)
        for b in range(B):
            beta[b, x_len[b]-1, y_len[b]] = x[b, x_len[b]-1, y_len[b], blank_idx]
            for t in range(x_len[b]-2, -1, -1):
                beta[b, t, y_len[b]] = beta[b, t+1, y_len[b]] + x[b, t, y_len[b], blank_idx] 
            for u in range(y_len[b]-1, -1, -1):
                beta[b, x_len[b]-1, u] = beta[b, x_len[b]-1, u+1] + x[b, x_len[b]-1, u, label[b, u]]
            for t in range(x_len[b]-2, -1, -1):
                for u in range(y_len[b]-1, -1, -1):
    ...
    def backward(x, label, x_len, y_len, alpha, beta, loss_grad, blank_idx):
        ...
        for b in range(B):
            ...
            for u in range(y_len[b]):
                grad[b, :x_len[b], u, label[b, u]] = -torch.exp(common_factor[b, :x_len[b], u] 
                                                        + beta[b, :x_len[b], u+1] 
                                                        + x[b, :x_len[b], u, label[b, u]])

            # current
            grad[b, :x_len[b]-1, :y_len[b]+1, blank_idx] \
                = -torch.exp(common_factor[b, :x_len[b]-1, :y_len[b]+1] 
                    + beta[b, 1:x_len[b], :y_len[b]+1] 
                    + x[b, :x_len[b]-1, :y_len[b]+1, blank_idx])

            grad[b, x_len[b]-1, y_len[b], blank_idx] = -torch.exp(common_factor[b, x_len[b]-1, y_len[b]]
                                                         + x[b, x_len[b]-1, y_len[b], blank_idx])
     
        return grad

    x_log = torch.nn.functional.log_softmax(x, dim=-1)
    alpha = forward_alpha(x_log, label, x_len, y_len, blank_idx)
    beta = forward_beta(x_log, label, x_len, y_len, blank_idx)
    grad = backward(x_log, label, x_len, y_len, alpha, beta, 
                        loss_grad, blank_idx)
    x_log.backward(grad)
    loss = -beta[:, 0, 0]
