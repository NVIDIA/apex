...
def check_checkpoint_args(checkpoint_args, args_value, vocab_file):
...
def read_metadata(tracker_filename):
    ...
    # NOTE (mkozuki): `rank` is undefined. But Megatron-LM repository has the same problem.
...
def load_checkpoint(model, optimizer, lr_scheduler, load_arg="load", strict=True):
    ...
    # NOTE (mkozuki): We only use PyTorch native AMP functionalities.
...
def load_biencoder_checkpoint(
    model, load, only_query_model=False, only_context_model=False, custom_load_path=None
):
    """
    ...
    load_path = custom_load_path if custom_load_path is not None else load
